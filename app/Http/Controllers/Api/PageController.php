...
class PageController extends Controller
{
    ...
    public function getMenuList(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $results = DB::select(DB::raw("select id,name, ( 6371 * acos( cos( radians(" . $lat1 . ") ) * cos( radians(latitude ) ) * cos( radians(longitude) - radians(" . $lon1 . ")) + sin(radians(" . $lat1 . ")) * sin( radians(latitude)))) AS distance from restaurants where id=" . $request->restaurant_id . "  order by distance limit 1"));
            if ($results && $results[0]->distance <= $distance) {
                $category_id = $request->category_id;
                $category_row = Category::where('id', $category_id)->first();
                ...
                if ($category_row) {
                    ...
                    if ($category_id == '1' || $category_id == '5') {
                        $checkchildcat = CategoryRelation::with('getRelativeCategorysData')->where('category_relations.parent_id', $category_id)->where('categories.restaurant_id', $request->restaurant_id)
                            ->join('categories', 'categories.id', '=', 'category_relations.category_id')
                            ->get();
                        foreach ($checkchildcat as $sub_category) {
                ...
                return response()->json(['status' => false, 'message' => 'To view the Menu, you have to be within a radius of 1 Km from any Brew Bistro Restaurant']);
            }
        }
    }
            ...
            if ($user->role_id == '4') {
                ...
                foreach ($all_tables as $table) {
                    if (isset($all_assigned_table[$table->id]) && $all_assigned_table[$table->id] == $user->id) {
                        ...
                        $table_array[$counter]['bill_narration'] = getBillCommentByTableId($table->id);
                        $counter++;
                    }
                }
            }
    ...
    public function setRatingByUser(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            //Y for yes N for Skipped
            'user_id' => 'required',
            'rating_for' => 'required',
            //O for order and R for Restro
    ...
    public function getUserNotification(Request $request)
    {
        ...
        $c = 0;
        foreach ($rows as $row) {
            $data = json_decode($row, true); // Convert JSON to PHP array with associative option
            $notification_rows[$c]['id'] = $row->id;
            $notification_rows[$c]['title'] = $row->title;
            $notification_rows[$c]['message'] = $row->message;
            $notification_rows[$c]['created_at'] = $data["created_at"];
            $notification_rows[$c]['is_seen'] = $row->is_seen == '1' ? true : false;
            $c++;
        }
        $response_array = ['status' => true, 'message' => 'list response', 'notifications' => $rows];
        $c = 0;
        foreach ($rows as $row) {
            $data = json_decode($row, true); // Convert JSON to PHP array with associative option
            $notification_rows[$c]['id'] = $row->id;
    ...
    public function getSalesManStatistics(Request $request): JsonResponse
    {
        $payload = [
            'status' => true,
            'message' => 'success',
        ...
        try {
            ...
            $payload['shops'] = WaRouteCustomer::where("route_id", $user->route)->count();
            $payload['unpaidOrders'] = WaInternalRequisition::where('route_id', $user->route)->where('status', '=', 'COMPLETED')->count();
            $totalOrders = WaInternalRequisition::where('route_id', $user->route)->get();

            $today = Carbon::now()->toDateString();
            $totalOrdersToday = WaInternalRequisition::where('route_id', $user->route)
                ->whereDate('created_at', '=', Carbon::parse($today))
                ->get();
            $payload['totalOrders'] = count($totalOrders);
            ...
            $orderTotalToday = 0;
            foreach ($totalOrdersToday as $order) {
                $orderTotalToday += $order->getOrderTotal();
            }

            $payload['totalOrdersAmount'] = $orderTotal;
            $payload['totalOrdersAmount'] = "KES. " . number_format($payload['totalOrdersAmount'], 2);

            if ($orderTotalToday > 0) {
                $route = Route::find($user->route);
                $percentage = floor(($orderTotalToday / $route->sales_target) * 100);
                $payload['target_percentage'] = $percentage;

                if ($payload['target_percentage'] > 100) {
                    $payload['target_percentage'] = 100;
                }
            }

//            $route = Route::find($user->route);
//            $salesTarget = $route->sales_target;
//            $currentDayOfWeek = Carbon::now()->dayOfWeek;
//            if ($route->order_taking_day == $currentDayOfWeek) {
//                $today = Carbon::now()->toDateString();
...
//                $orderTotal = 0;
//
//                // TODO: Optimize (HasManyThrough)
//                foreach ($orders as $order) {
//                    foreach ($order->getRelatedItem as $item) {
//                        $orderTotal += ((float)$item->quantity * (float)$item->selling_price);
//                    }
//                }
//
//                if ($orderTotal > 0) {
//                    $percentage = floor(($orderTotal / $salesTarget) * 100);
//                    $payload['target_percentage'] = $percentage;
//                }
//            }
    ...
    public function getDeliveryManStatistics(Request $request): JsonResponse
    {
        try {
            $user = JWTAuth::toUser($request->token);


            $delivery = DeliverySchedule::with(['customers','route','route.waRouteCustomer'])->latest()->forDriver($user->id)->first();
            $payload = [
                'routes' => 0,
                "totalDeliveries" => 0,
                'totalOrdersAmount' => 0,
                'shops_count' => $delivery->route->centers()->count() . " Shops",
                'centres_count' => $delivery->route->waRouteCustomer()->count() . " Centers",
                'centres_to_visit_count' => @$delivery->route->centers->count() . " Centers",
                'shops_to_visit_count' => @$delivery->customers->count() . " Shops",
            ];
            if ($delivery) {
                $payload['routes'] = 1;
                $payload['totalDeliveries'] = $delivery->customers()->count();
                $payload['totalOrdersAmount'] = 0;
                $payload['totalDeliveredAmount'] = 0;
                $payload['totalUndeliveredAmount'] = 0;

                foreach ($delivery->customers as $customer) {
                    $order = WaInternalRequisition::latest()
                        ->where('wa_route_customer_id', $customer->customer_id)
                        ->where('wa_shift_id', $delivery->shift_id)
                        ->first();
                    $payload['totalOrdersAmount'] += $order->getOrderTotal();
                }
                foreach ($delivery->customers->where('visited', true) as $customer) {
                    $order = WaInternalRequisition::latest()
                        ->where('wa_route_customer_id', $customer->customer_id)
                        ->where('wa_shift_id', $delivery->shift_id)
                        ->first();
                    $payload['totalDeliveredAmount'] += $order->getOrderTotal();
                }
            }

            $payload['totalUndeliveredAmount'] = format_amount_with_currency($payload['totalOrdersAmount']- $payload['totalDeliveredAmount']);
            $payload['totalOrdersAmount'] = format_amount_with_currency($payload['totalOrdersAmount']);
            $payload['totalDeliveredAmount'] = format_amount_with_currency($payload['totalDeliveredAmount']);
            return $this->jsonify([...$payload, ...['status' => true]], 200);
        } catch (\Throwable $e) {
