...
class TyreTransferController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->model = 'tyre_transfer';
        $this->title = 'Tyre Transfer';
        $this->pmodule = 'tyre_transfer';
    } 
    ...
    public function tyre_transfer_dropdown(Request $request){
    ...
    public function create(Request $request){
        ...
        if(isset($permission[$pmodule.'___add']) || $permission == 'superadmin'){
            ...
            return view('admin.tyre_transfer.create',compact('title','model','breadcum','pmodule','permission','tyre_transfer','tyre_serials','vehicle_reg_no','odometer'));
        }
        else{
            Session::flash('warning', 'Invalid Request');
    ...
    public function store(Request $request){
        ...
        $last_odometer_reading=checkEnforceMeterReading($request->vehicle,'tyre_transfer');
        ...
        $transfer_status_arr=$request->transfer_status;
        $new_vehicle_arr=$request->new_vehicle;
        $tyre_position_id_arr=$request->tyre_position_id;

        //dd($request->all());

        $loggedUserData=getLoggeduserProfile();
        if(!empty($new_vehicle_arr)){
            foreach($new_vehicle_arr as $tyre_serial_id => $new_vehicle_id){
                ...
                if($new_vehicle_id!=null){
                    ...
                    $findSerial->vehicle_id=$new_vehicle_id;
                    ...
                    $pssm_history->status='transfer';
                    $pssm_history->vehicle_id=$new_vehicle_id;
                    $pssm_history->save();   
                }
            }    
            
            saveOdometerHistory($request->vehicle,$request->odometer,'tyre_transfer');
            

            return response()->json([
                'result'=>1,
                'location'=>route($this->model.'.create',$param_arr),
                'message'=>'Tyre Transfer successfully'
            ]);
            ...
            return response()->json([
                'result'=>-1,
                'location'=>route($this->model.'.create',$param_arr),
                'message'=>'Must be select 1 Vehicle Atleast!'
            ]);
        }
        ...
        try
        {
            ...
            if(isset($permission[$this->pmodule.'___edit']) || $permission == 'superadmin')
            {
                ...
                if($row)
                {
                    ...
                    return view('admin.tyre_transfer.edit',compact('title','model','breadcum','row','tyre_transfer','make','modal','bodytype')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
      ...
      public function update(Request $request, $id)
    {
        ...
        {  
            $row =  Vehicle::where('id',$id)->first();
            // $row->tyre_transfer_name= $request->tyre_transfer_name;
            ...
            if ($request->hasFile('photo'))
                  {
                      ...
                      $image = uploadwithresize($file, 'tyre_transferlist');
                      $row->photo = $image;
                  }
        ...
        try
        {
            ...
            if(isset($permission[$this->pmodule.'___view']) || $permission == 'superadmin')
            {
                ...
                if($row)
                {
                    ...
                    $insepection_history=InspectionHistory::select('inspection_history_items.item_detail')->leftjoin('inspection_history_items','inspection_history.id','=','inspection_history_items.inspection_history_id')->where('tyre_transfer_id',$id)->get();
                    ...
                    return view('admin.tyre_transfer.show',compact('title','model','breadcum','row','tyre_transfer','make','modal','bodytype','issueOpenCount','issueOverDueCount','remaindersOverdueCount','remaindersSnoozedCount','remaindersDueCount','inspectionPassCount','inspectionFailCount')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
