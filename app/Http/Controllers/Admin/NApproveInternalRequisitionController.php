...
use App\Model\NWaInternalReqPermissionDemo;


use App\Model\NWaInternalRequisitionDemo;
use App\Model\NWaInternalRequisitionItemDemo;
...
class NApproveInternalRequisitionController extends Controller
{
    ...
    {
        $this->model = 'n-authorise-requisitions';
    ...
    public function index()
    {
        ...
        if(isset($permission[$pmodule.'___view']) || $permission == 'superadmin')
        {
            ...
            $lists = NWaInternalReqPermissionDemo::where('user_id',getLoggeduserProfile()->id)->where('status','NEW')->orderBy('id', 'desc')->get();
            $breadcum = [$title=>route($model.'.index'),'Listing'=>''];
            return view('admin.Napproveinternalrequisition.index',compact('title','lists','model','breadcum','pmodule','permission'));
        }
        else
        {
    ...
    public function edit($slug)
    {
        try
        {
                ...
                if($row)
                {
                    ...
                    $model =$this->model;
                    return view('admin.Napproveinternalrequisition.edit',compact('title','model','breadcum','row')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
    ...
    public function update(Request $request, $slug)
    {
        ...
        try
        {
            ...
            {
                $permmission_detail = NWaInternalReqPermissionDemo::where('wa_internal_requisition_id',$row->id)->where('user_id',$logged_user->id)->update(['status'=>'APPROVED','note'=>$request->authorizer_note]);
                 
                 $lattest_permmission = NWaInternalReqPermissionDemo::where('wa_internal_requisition_id',$row->id)->where('approve_level','>',$logged_user->authorization_level)->orderBy('approve_level','asc')->first();
                 ...
                 if($lattest_permmission)
                {
                    ...
                    $ex_req =  NWaInternalRequisitionDemo::where('id',$row->id)->first();
            ...
            {
                //decline case
               NWaInternalReqPermissionDemo::where('wa_internal_requisition_id',$row->id)->where('user_id',$logged_user->id)->update(['status'=>'DECLINED','note'=>$request->authorizer_note]);

                NWaInternalReqPermissionDemo::where('wa_internal_requisition_id',$row->id)->where('approve_level','>',$logged_user->authorization_level)->delete();
                $row->status = 'DECLINED';
                $row->save();
            }
    ...
    public function deletingItemRelation($purchase_no,$id)
    {
        try
        {
            NWaInternalRequisitionItemDemo::whereId($id)->delete();
    ...
    public function editPurchaseItem($purchase_no,$id)
    {
        try
        {
                ...
                $row =  NWaInternalRequisitionDemo::where('requisition_no',$purchase_no)
                ...
                if($row)
                {
                    ...
                    return view('admin.Napproveinternalrequisition.editItem',compact('title','model','breadcum','row','id','form_url')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
    ...
    public function updatePurchaseItem(Request $request,$id)
    {
        try
        {
            ...
            $item =  NWaInternalRequisitionItemDemo::where('id',$id)->first();
