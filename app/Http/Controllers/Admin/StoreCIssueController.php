...
use App\Model\WaStoreCRequisition;
use App\Model\WaStoreCRequisitionItem;
...
class StoreCIssueController extends Controller
{
    protected $model;
    protected $title;
     protected $pmodule;
    ...
    {
        $this->model = 'store-c-issue';
        $this->title = 'Store C Issue';
        $this->pmodule = 'store-c-issue';
    } 
    ...
    public function index()
    {
        ...
        {
            $lists = WaStoreCRequisition::where('status','APPROVED')->with(['getrelatedEmployee','getBranch','getDepartment','getRelatedItem','getRelatedToLocationAndStore']);
          ...
          $lists = $lists->orderBy('id', 'desc')->get();
            ...
            return view('admin.store_c_issue.index',compact('title','lists','model','breadcum','pmodule','permission'));
        }
        else
        {
    ...
    public function edit($slug)
    {
        ...
        try
        {            
            $row =  WaStoreCRequisition::with(['getRelatedItem',
            ...
            if($row)
            {
                ...
                $model =$this->model;
                return view('admin.store_c_issue.edit',compact('title','model','breadcum','row')); 
            }
            else
            {
                Session::flash('warning', 'Invalid Request');
    ...
    public function update(Request $request,$slug)
    {
        ...
        $validate = Validator::make($request->all(),[
            'item_id.*'=>'required|exists:wa_store_c_requisition_items,id',
        ...
        $check = DB::transaction(function () use ($request,$row) {
            ...
            $series_module = \App\Model\WaNumerSeriesCode::where('module', 'Internal Requisition Store C')->first();
            foreach($row->getRelatedItem as $value){
                $stock_qoh = $value->getInventoryItemDetail->getAllFromStockMovesC->where('wa_location_and_store_id',$value->store_location_id)->sum('qauntity');
                $stock_qoh -= @$request->item_quantity[$value->id];
                $stockMove = new WaStockMoveC();
                $stockMove->user_id = $getLoggeduserProfile->id;
                $stockMove->wa_store_c_requisitions_id = $row->id;
                ...
                $stockMove->save();
                WaStoreCRequisitionItem::where('id',$value->id)->update(['issued_quanity'=>@$request->item_quantity[$value->id]]);
            }
    ...
    public function completedIndex()
    {
        ...
        {
            $lists = WaStoreCRequisition::where('status','COMPLETED')
            ->with(
                ['getrelatedEmployee','getBranch','getDepartment','getRelatedItem','getRelatedToLocationAndStore'])
            ...
            //     $lists = $lists->where('user_id', getLoggeduserProfile()->id);
            // }
          $lists = $lists->orderBy('id', 'desc')->limit(10)->get();
            $breadcum = [$title=>route('store-c-requisitions.completedIndex'),'Listing'=>''];
            return view('admin.store_c_issue.completedIndex',compact('title','lists','model','breadcum','pmodule','permission'));
        }
        else
        {
            Session::flash('warning', 'Invalid Request');
    ...
    public function inventoryItems(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            ->join('wa_inventory_categories','wa_inventory_categories.id','=','wa_inventory_items.wa_inventory_category_id')->where('wa_inventory_items.store_c_deleted',0);
           ...
           // dd($data_query);
            if (!empty($data_query)) {
                foreach ($data_query as $key => $row) {
                    ...
                    $action_text = '<a href="'.route('store-c-issue.stock-movements',$row->stock_id_code).'" class="btn btn-sm btn-biz-greenish" title="stock movement"><i class="fa fa-list"></i></a>';
                    if(isset($permission['store-c-inventory___delete']) || $permission == 'superadmin'){
                        $action_text .= '<a href="'.route('store-c-issue.inventory-item-delete',$row->id).'" class="btn btn-sm btn-biz-pinkish deleteMe" title="inventory item archive"><i class="fa fa-archive"></i></a>';
                    }
                    if(isset($permission['store-c-inventory___adjustment']) || $permission == 'superadmin'){
        ...
        if(isset($permission['store-c-inventory___view']) || $permission == 'superadmin')
        {
            $breadcum = [$title=>route('store-c-issue.inventoryItems'),'Listing'=>''];
            return view('admin.store_c_issue.inventoryitem',compact('title','model','breadcum','pmodule','permission'));
        }
        else
        {
            Session::flash('warning', 'Invalid Request');
    ...
    public function inventoryItem_delete(Request $request,$id)
    {
        ...
        try {
            ...
            $item = WaInventoryItem::where('id',$id)->where('wa_inventory_items.store_c_deleted',0)->first();
            if($item){
                $item->store_c_deleted = 1;
                $item->save();
    ...
    public function stockMovements($StockIdCode,Request $request)
    {
        ...
        $breadcum = [$title=>route('store-c-issue.inventoryItems'),'Stock Movement'=>'',$StockIdCode=>''];
        if($request->type == 'pdf'){
            ...
            $pdf = \PDF::loadView('admin.store_c_issue.stockmovement_pdf',compact('firstQoh','currentLocation','request','row','location','title','lists','model','breadcum','pmodule','permission', 'StockIdCode'));
        ...
        if($request->type == 'print'){
            ...
            ->where('stock_id_code',$StockIdCode)->orderBy('id', 'DESC')->first();
            $currentLocation = WaLocationAndStore::where(['id'=>$request->location])->first();
            return view('admin.store_c_issue.stockmovement_pdf',compact('firstQoh','currentLocation','request','row','location','title','lists','model','breadcum','pmodule','permission', 'StockIdCode'));
        }
        return view('admin.store_c_issue.stockmovement',compact('location','title','lists','model','breadcum','pmodule','permission', 'StockIdCode'));
    }


    public function stock_take_index() {
        $permission = $this->mypermissionsforAModule();
        $pmodule = 'store-c-stock-take';
        ...
        if (isset($permission[$pmodule . '___view']) || $permission == 'superadmin') {
            ...
            return view('admin.store_c_stock_takes.index', compact('title', 'lists', 'model', 'breadcum', 'pmodule', 'permission'));
        } else {
            Session::flash('warning', 'Invalid Request');
            return redirect()->back();
        }
...
//        dd($branch_id);
    ...
    public function addStockCheckFile(Request $request){
        ...
        DB::transaction(function () use ($request){
            ...
            $generateQuery = \App\Model\WaInventoryItem::with(['getUnitOfMeausureDetail','getInventoryCategoryDetail'])->groupBy('wa_unit_of_measure_id','store_location_id','wa_inventory_category_id')->where('store_c_deleted',0)->get();
            // $allItems = \App\Model\WaInventoryItem::where('store_c_deleted',0)->with(['getAllFromStockMovesC'])->get();
            $logged_user_profile = getLoggeduserProfile();
            ...
            {
                $items = \App\Model\WaInventoryItem::where('store_c_deleted',0)->with(['getAllFromStockMovesC'])->where('wa_unit_of_measure_id',$query->wa_unit_of_measure_id)->where('store_location_id',$query->store_location_id)->where('wa_inventory_category_id', $query->wa_inventory_category_id)->get();
    ...
    public function printToPdf($id) {
       ...
       $pdf = PDF::loadView('admin.store_c_stock_takes.print', compact('items_by_category', 'category_list', 'data'));
    ...
    public function printPage(Request $request)
    {
        ...
        $print_page = 1;
        return view('admin.store_c_stock_takes.print',compact('print_page', 'items_by_category', 'category_list', 'data', 'title','model','breadcum')); 
    }
    ...
    public function archivedinventoryItems(Request $request)
    {
        ...
        if($request->ajax()){
            ...
            //$data_query = WaInventoryItem::select('items.*')->where([['type', $item_types['feed']]]);
            $data_query = WaInventoryItem::select('wa_inventory_categories.id as cat_id','wa_inventory_categories.category_description','wa_inventory_items.*')->with('pack_size','getUnitOfMeausureDetail', 'getAllFromStockMovesC')
            ->join('wa_inventory_categories','wa_inventory_categories.id','=','wa_inventory_items.wa_inventory_category_id')->where('wa_inventory_items.store_c_deleted',1);
            if (!empty($request->input('search.value'))) {
                $search = $request->input('search.value');
                $data_query = $data_query->where(function($data_query) use ($search) {
                    $data_query->where('stock_id_code', 'LIKE', "%{$search}%")
           ...
           // dd($data_query);
            if (!empty($data_query)) {
                foreach ($data_query as $key => $row) {
                    ...
                    $action_text = '';
                    if(isset($permission['store-c-inventory___delete']) || $permission == 'superadmin'){
                        $action_text .= '<a href="'.route('store-c-issue.inventory-item-un-archive',$row->id).'" class="btn btn-sm btn-biz-greenish deleteMe" title="inventory item un-archive"><i class="fa fa-archive"></i></a>';
                    }
        ...
        if(isset($permission['store-c-inventory___view']) || $permission == 'superadmin')
        {
            $breadcum = [$title=>route('store-c-issue.archivedinventoryItems'),'Listing'=>''];
            return view('admin.store_c_issue.archivedinventoryItems',compact('title','model','breadcum','pmodule','permission'));
        }
        else
        {
            Session::flash('warning', 'Invalid Request');
    ...
    public function archived_inventoryItem_delete(Request $request,$id)
    {
        ...
        try {
            ...
            $item = WaInventoryItem::where('id',$id)->where('wa_inventory_items.store_c_deleted',1)->first();
            if($item){
                $item->store_c_deleted = 0;
                $item->save();
