...
class ReorderItemsReportController extends Controller
{
    protected $model;
    ...
    {
        $this->model = 'reorder-items-report';
        $this->title = 'Reorder Items Report';
    }

    public function index()
    {
        if (!can('reorder-items-report', 'inventory-reports')) {
        ...
        $query = WaInventoryItem::query()
            ...
            ->whereColumn('stocks.quantity', '<=', 'max_stocks.re_order_level')
            ->whereRaw('stocks.quantity > 0')
        ...
        if (request()->action) {
            ...
            if (request()->action == 'excel') {
                $view = view('admin.reports.export.reorder_items_report', [
                ...
                return Excel::download(new ExportViewToExcel($view), 'reorder_items_report_' . date('YmdHis') . '.xlsx');
            }
        }
        ...
        return view('admin.reports.reorder_item_report', [
            'model' => $this->model,
            'title' => $this->title,
