...
use App\Model\WaCreditNote;
use App\Model\WaCustomer;
use App\Model\WaCreditNoteItem;
...
class CreditNoteController extends Controller
{
    ...
    {
        $this->model = 'credit-note';
        $this->title = 'Credit Note';
        $this->pmodule = 'credit-note';
    }
        ...
        if (isset($permission[$pmodule . '___view']) || $permission == 'superadmin') {
            if ($permission != 'superadmin') {
                if (isset($permission[$pmodule . '___view-all'])) {
                    $lists = WaCreditNote::orderBy('id', 'desc')->get();
                } else {
                    // $lists = WaCreditNote::where('creater_id', getLoggeduserProfile()->id);
            ...
            return view('admin.creditnote.index', compact('title', 'lists', 'model', 'breadcum', 'pmodule', 'permission'));
        } else {
    ...
    public function create()
    {
        if (getLoggeduserProfile()->wa_department_id && getLoggeduserProfile()->restaurant_id) {
            ...
            if (isset($permission[$this->pmodule . '___add']) || $permission == 'superadmin') {
                ...
                return view('admin.creditnote.create', compact('title', 'model', 'breadcum'));
            } else {
                Session::flash('warning', 'Invalid Request');
                return redirect()->back();
            }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $row = new WaCreditNote();
            $row->credit_note_number = $request->credit_note_number;
            ...
            $row->save();
            updateUniqueNumberSeries('CREDIT_NOTE', $request->credit_note_number);



            $item = new WaCreditNoteItem();
            $item->wa_credit_note_id = $row->id;
    ...
    public function show($slug)
    {
        ...
        $row =  WaCreditNote::whereSlug($slug)->first();
        if ($row) {
            $title = 'View ' . $this->title;
            $breadcum = [$this->title => route($this->model . '.index'), 'Show' => ''];
            $model = $this->model;
            return view('admin.creditnote.show', compact('title', 'model', 'breadcum', 'row'));
        } else {
            Session::flash('warning', 'Invalid Request');
            return redirect()->back();
        }
    ...
    public function print(Request $request)
    {
        ...
        $breadcum = [$this->title => route($model . '.index'), 'Add' => ''];
        $row =  WaCreditNote::whereSlug($slug)->first();
        return view('admin.creditnote.print', compact('title', 'model', 'breadcum', 'row'));
    }
    ...
    public function exportToPdf($slug)
    {
        ...
        $breadcum = [$this->title => route($model . '.index'), 'Add' => ''];
        $row =  WaCreditNote::whereSlug($slug)->first();
        $pdf = PDF::loadView('admin.creditnote.print', compact('title', 'model', 'breadcum', 'row'));
        $report_name = 'CREDIT_NOTE_' . date('Y_m_d_H_i_A');
    ...
    public function edit($slug)
    {
        try {
            ...
            if ($row) {
                ...
                $model = $this->model;
                return view('admin.creditnote.edit', compact('title', 'model', 'breadcum', 'row'));
            } else {
                Session::flash('warning', 'Invalid Request');
                return redirect()->back();
            }
    ...
    public function process(Request $request, $slug)
    {
        ...
        try {
            ...
            $series_module = WaNumerSeriesCode::where('module', 'CREDIT_NOTE')->first();
            ...
            foreach ($row->getRelatedItem as $item) {
                ...
                if (isset($request->$key)) {
                    ...
                    if ($discount_percent > 0) {
                        ...
                        $itemRow = WaCreditNoteItem::where('id', $item->id)->first();
            ...
            $invoice_items = WaCreditNoteItem::where('wa_credit_note_id', $row->id)
            ...
            $sale_invoiceno = getCodeWithNumberSeries('CREDIT_NOTE');
            ...
            foreach ($invoice_items as $invoice_item) {
                ...
                if ($invoice_item->item_type != "gl-code") {
                    ...
                    $stockMove->document_no =   $row->credit_note_number;
                ...
                $cr->transaction_no = $row->credit_note_number;
            ...
            if (count($total_invoice_amount) > 0) {
                ...
                $dr->transaction_no = $row->credit_note_number;
    ...
    {
        $accountuingPeriod =  WaAccountingPeriod::where('is_current_period', '1')->first();
        $invoice =  WaCreditNote::where('id', $invoiceId)->first();
        $series_module = WaNumerSeriesCode::where('module', 'CREDIT_NOTE')->first();
        $debtorTran = new WaDebtorTran();
        $debtorTran->wa_credit_note_id = $invoiceId;
        ...
        $debtorTran->document_no = $invoice->credit_note_number;
        $debtorTran->save();
    }
    ...
    {
        try {
            WaCreditNote::whereSlug($slug)->delete();
