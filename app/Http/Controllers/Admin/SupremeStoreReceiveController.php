...
use App\Model\WaSupremeStoreReceive;
use App\Model\WaSupremeStoreReceiveItems;
use App\Model\WaStockMoveC;
use Session;
use DB;
class SupremeStoreReceiveController extends Controller
{
    protected $model;
    protected $title;
    protected $pmodule;
    ...
    {
        $this->model = 'supreme-store-receive';
        $this->title = 'Supreme Store Receive';
        $this->pmodule = 'supreme-store-receive';
    }
    ...
    public function index(Request $request)
    {
        ...
        if (isset($permission[$pmodule . '___view']) || $permission == 'superadmin') {
            ...
            if($request->ajax()){
                $sortable_columns = [
                    'wa_supreme_store_receives.id',
                    'users.name',
                    'wa_supreme_store_receives.receive_code',
                    // 'wa_supreme_store_receives.user_id',
                ];
                ...
                $draw           = $request['draw'];          
                $data = WaSupremeStoreReceive::with(['items'])->select(['wa_supreme_store_receives.*','users.name as user_name'])->where(function($w) use ($user,$request,$search){
                    ...
                    if($search){
                        $w->orWhere('wa_supreme_store_receives.receive_code','LIKE',"%$search%");
                        $w->orWhere('users.name','LIKE',"%$search%");
                    }
                })->leftjoin('users',function($join){
                    $join->on('users.id','=','wa_supreme_store_receives.user_id');
                ...
                $response       = $data->limit($limit)->offset($start)->get()->map(function($item) use ($permission,$user){
                    ...
                    if ($item->status == 'PENDING'){
                        $item->links .= '<a style="margin: 2px;" class="btn btn-warning btn-sm" href="'.route('supreme-store-receive.edit',base64_encode($item->id)).'" title="Details"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                    }
                    $item->links .= '<a style="margin: 2px;"  class="btn btn-danger btn-sm" href="'.route('supreme-store-receive.show',base64_encode($item->id)).'" title="Details"><i class="fa fa-eye" aria-hidden="true"></i></a>';
                    if($item->status != 'PENDING'){
                        $item->links .= '<a style="margin: 2px;"  class="btn btn-primary btn-sm printBill" onclick="printBill(this); return false;" href="'.route('supreme-store-receive.invoice_print',base64_encode($item->id)).'" title="Print"><i class="fa fa-print" aria-hidden="true"></i></a>';
                        //$item->links .= '<a style="margin: 2px;"  class="btn btn-warning btn-sm" href="'.route('supreme-store-receive.exportToPdf',base64_encode($item->id)).'" title="PDF">
                        ...
                        // if (($item->items->where('store_location_id',$user->wa_location_and_store_id)->where('is_return',0)->count() != 0 && $permission != 'superadmin') || ($permission == 'superadmin' && $item->items->where('is_return',0)->count() != 0)){
                            // $item->links .= '<a  style="margin: 2px;" class="btn btn-success btn-sm" href="'.route('supreme-store-receive.return_items',base64_encode($item->id)).'" title="Return">
            ...
            return view('admin.supreme_store_receive.index', compact('user','title', 'model', 'breadcum', 'pmodule', 'permission'));
        } else {
    ...
    public function create(Request $request)
    {
        ...
        if (isset($permission[$pmodule . '___add']) || $permission == 'superadmin') {
            $breadcum = [$title => route($model . '.index'), 'Listing' => ''];
            return view('admin.supreme_store_receive.create', compact('title', 'model', 'breadcum', 'pmodule', 'permission'));
        } else {
            Session::flash('warning', 'Invalid Request');
            return redirect()->back();
        }
    ...
    public function getInventryItemDetails(Request $request)
    {
        ...
        $data = WaInventoryItem::select([
            ...
            if($request->supreme_store){
                $e->where('supreme_store_deleted',0);
            }
    ...
    public function store(Request $request)
    {
        ...
        $check = DB::transaction(function () use ($allInventroy,$request){
            ...
            $parent = new WaSupremeStoreReceive;
            ...
            foreach ($allInventroy as $key => $value) {
                ...
                $data = [];
                $data['wa_supreme_store_receive_id'] = $parent->id;
                ...
                if($request->request_type == 'send_request'){
                    ...
                    $stockMove->wa_supreme_store_receive_id = $parent->id;
            ...
            WaSupremeStoreReceiveItems::insert($childs);
        ...
        if($check){
            if($request->request_type == 'send_request'){
                ...
                $location = route('supreme-store-receive.index');
            }else {
    ...
    public function edit($id)
    {
        ...
        if (isset($permission[$pmodule . '___edit']) || $permission == 'superadmin') {
            $breadcum = [$title => route($model . '.index'), 'Listing' => ''];
            $getLoggeduserProfile = getLoggeduserProfile();
            $data = WaSupremeStoreReceive::with(['items','user','items.item','items.item.getAllFromStockMovesC'=>function($w) use ($getLoggeduserProfile){
            ...
            return view('admin.supreme_store_receive.edit', compact('title', 'model', 'breadcum', 'pmodule', 'permission','data','editPermission'));
        } else {
            Session::flash('warning', 'Invalid Request');
            return redirect()->back();
        }
    ...
    public function update(Request $request,$id)
    {
        ...
        $validation = \Validator::make($request->all(),[
            ...
            'item_quantity.*'=>'required|min:1|numeric',
            'time'=>'required',
            'id'=>'required|exists:wa_supreme_store_receives,id',
            'request_type'=>'required|in:send_request,save'
        ],[
            'item_quantity.*.min'=>'Quantity must be greater than or equal to 1',
        ],[
        ...
        $parent = WaSupremeStoreReceive::where('id',$request->id)->first();
        ...
        $check = DB::transaction(function () use ($allInventroy,$request,$parent){
            ...
            $total_vat_amount = 0;
            $dateTime = date('Y-m-d H:i:s');
            WaSupremeStoreReceiveItems::where('wa_supreme_store_receive_id',$parent->id)->delete();
            foreach ($allInventroy as $key => $value) {
                $stock_qoh = $value->getAllFromStockMovesC->where('wa_location_and_store_id',$value->store_location_id)->sum('qauntity') ?? 0;
                $data = [];
                $data['wa_supreme_store_receive_id'] = $parent->id;
    ...
    public function show($id)
    {
        ...
        if (isset($permission[$pmodule . '___view']) || $permission == 'superadmin') {
            $breadcum = [$title => route($model . '.index'), 'Listing' => ''];
            $data = WaSupremeStoreReceive::with(['items','user','items.item','items.item.pack_size','items.location'])->where('id',$id)->first();
            if(!$data){
                Session::flash('warning','Invalid Request');
                return redirect()->back();
            }
            return view('admin.supreme_store_receive.show', compact('data','title', 'model', 'breadcum', 'pmodule', 'permission'));
        } else {
            Session::flash('warning', 'Invalid Request');
            return redirect()->back();
        }
    ...
    public function invoice_print($id)
    {
        ...
        return view('admin.supreme_store_receive.print', compact('data','title', 'model','pmodule', 'permission'));
    }
    ...
    public function exportToPdf($id)
    {
        ...
        $title = $this->title;
        $model = $this->model;
        $data = WaSupremeStoreReceive::with(['items','user','items.item','items.item.pack_size','items.location','items.dispatch_by'])->where('id',$id)->first();
        if(!$data){
            Session::flash('warning','Invalid Request');
            return redirect()->back();
        }
        $pdf = \PDF::loadView('admin.supreme_store_receive.print', compact('data','title', 'model','pmodule', 'permission'));
        $report_name = 'supreme_store_receive_'.date('Y_m_d_H_i_A');
