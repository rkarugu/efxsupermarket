...
class StoreCRequisitionController extends Controller
{
    ...
    {
        $this->model = 'store-c-requisitions';
        $this->title = 'Store C Reqeust';
        $this->pmodule = 'store-c-requisitions';
    } 
    ...
    public function index()
    {
        ...
        {
            $lists = WaStoreCRequisition::where('status','!=','COMPLETED')->with(['getrelatedEmployee','getRelatedToLocationAndStore','getDepartment','getRelatedItem']);
          ...
          $lists = $lists->orderBy('id', 'desc')->get();
            ...
            return view('admin.store_c_requisition.index',compact('title','lists','model','breadcum','pmodule','permission'));
        }
        else
        {
    ...
    public function create()
    {
        if(getLoggeduserProfile()->wa_department_id && getLoggeduserProfile()->restaurant_id)
        {
            ...
            if(isset($permission[$this->pmodule.'___add']) || $permission == 'superadmin')
            {
                ...
                return view('admin.store_c_requisition.create',compact('title','model','breadcum'));
            }
            else
            {
                Session::flash('warning', 'Invalid Request');
    ...
    public function create2(){
        ...
        {
            $permission =  $this->mypermissionsforAModule();
            if(isset($permission[$this->pmodule.'___add-2']) || $permission == 'superadmin'){
                $title = 'Add '.$this->title;
                $model = $this->model;
                $breadcum = [$this->title =>route($model.'.index'),'Add'=>''];
                $dispatchLoadingSheetData = DispatchLoadedProducts::with(['getSalesMan','getShift','getStoreLocation','getInventoryItem'])->groupBy('shift_id','store_location_id','salesman_id')
                ->where('is_requisition_done',0)
                ->where('document_no',null)
                ->get();

                $dispatchInvoiceData=DispatchLoadedProducts::with(['getSalesMan','getShift','getStoreLocation','getInventoryItem'])->groupBy('shift_id','store_location_id','document_no')
                ->where('is_requisition_done',0)
                ->where('shift_id',null)
                ->where('balance_qty','>','0')
                ->get();
                ...
                return view('admin.store_c_requisition.create2',compact('title','model','breadcum','dispatchLoadingSheetData','dispatchInvoiceData'));
            }
            else
            {
                Session::flash('warning', 'Invalid Request');
    ...
    public function print(Request $request)
    {
        ...
        $model = $this->model;
        $breadcum = [$this->title =>route($model.'.index'),'Add'=>''];
        $row =  WaStoreCRequisition::with(['getrelatedEmployee','getRelatedItem','getRelatedItem.getInventoryItemDetail','getRelatedItem.location','getRelatedItem.getInventoryItemDetail.getAllFromStockMovesC'])->whereSlug($slug)->first();
        return view('admin.store_c_requisition.print',compact('title','model','breadcum','row')); 
    }
    ...
    public function exportToPdf($slug)
    {
        ...
        $model = $this->model;
        $breadcum = [$this->title =>route($model.'.index'),'Add'=>''];
        $row =  WaStoreCRequisition::whereSlug($slug)->first();
        $pdf = PDF::loadView('admin.store_c_requisition.print', compact('title','model','breadcum','row'));
        $report_name = 'store_c__requisition_'.date('Y_m_d_H_i_A');
    ...
    public function store(Request $request){
        ...
        $check = DB::transaction(function () use ($allInventroy,$request){
            ...
            $sale_invoiceno = getCodeWithNumberSeries('Internal Requisition Store C');
            $getLoggeduserProfile = getLoggeduserProfile();
            $dateTime = date('Y-m-d H:i:s');
            $parent = new WaStoreCRequisition;
            ...
            foreach ($allInventroy as $key => $value) {
                ...
                $data = [];
                $data['wa_store_c_requisitions_id'] = $parent->id;
                ...
                if($request->request_type == 'send_request'){
                    ...
                    // $stockMove->wa_store_c_requisitions_id = $parent->id;
            ...
            WaStoreCRequisitionItem::insert($childs);
            
            if($request->request_type == 'send_request'){
                $parent->status = 'PENDING';
                $parent->save();
                addstore_c_RequisitionPermissions($parent->id,$parent->wa_department_id);
            }
            
            updateUniqueNumberSeries('Internal Requisition Store C',$parent->requisition_no);
            return $parent;
        });
        if($check){
            if($request->request_type == 'send_request'){
                ...
                $location = route('store-c-requisitions.index');
            }else {
    ...
    public function store2(Request $request){
        ...
        $check = DB::transaction(function () use ($allInventroy,$request){
            ...
            foreach ($allInventroy as $key => $value) {
             ...
             $id = explode("_",$request->idds);

                $singleRecord = array('is_requisition_done' => 1);
            $udaptes = DispatchLoadedProducts::where(
                [
                    'store_location_id' => $id[1],
                    'shift_id' => $id[0],
                    'salesman_id' => $id[2],
                ])->update($singleRecord);
    ...
    public function show($slug)
    {
            ...
            if($row)
            {
                ...
                $model =$this->model;
                return view('admin.store_c_requisition.show',compact('title','model','breadcum','row')); 
            }
            else
            {
                Session::flash('warning', 'Invalid Request');
    ...
    public function edit($slug)
    {
        ...
        try
        {
                ...
                if($row)
                {
                    ...
                    $model =$this->model;
                    return view('admin.store_c_requisition.edit',compact('title','model','breadcum','row')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
    ...
    public function update(Request $request, $slug)
    {
        ...
        $parent = WaStoreCRequisition::where('slug',$slug)->first();
        ...
        $check = DB::transaction(function () use ($allInventroy,$request,$parent){
            ...
            $parent->save();
            $childs = [];   
            WaStoreCRequisitionItem::where('wa_store_c_requisitions_id',$parent->id)->delete();
            foreach ($allInventroy as $key => $value) {
                $stock_qoh = $value->quantity ?? 0;
                $data = [];
                $data['wa_store_c_requisitions_id'] = $parent->id;
    ...
    public function destroy($slug)
    {
        ...
        try
        {
            $row = WaStoreCRequisition::whereSlug($slug)->first();
            if($row){
                WaStoreCRequisitionItem::where('wa_store_c_requisitions_id',$row->id)->delete();
                $row->delete();
            }
    ...
    public function deletingItemRelation($purchase_no,$id)
    {
        try
        {
            WaStoreCRequisitionItem::whereId($id)->delete();
    ...
    public function downloadPrint($requisition_no)
    {
         ...
         $row =  WaStoreCRequisition::where('requisition_no',$requisition_no)->first();

      $pdf = PDF::loadView('admin.store_c_requisition.print', compact('row'));
    ...
    public function editPurchaseItem($requisition_no,$id)
    {
        try
        {
                ...
                $row =  WaStoreCRequisition::where('requisition_no',$requisition_no)
                ...
                if($row)
                {
                    ...
                    return view('admin.store_c_requisition.editItem',compact('title','model','breadcum','row','id','form_url')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
    ...
    public function updatePurchaseItem(Request $request,$id)
    {
        try
        {
            ...
            $item =  WaStoreCRequisitionItem::where('id',$id)->first();
    ...
    public function inventoryItems(Request $request)
    {
        ...
        })->where('store_c_deleted',0)->having('quantity','>',0)->limit(20)->get();
    ...
    public function getInventryItemDetails(Request $request)
    {
        ...
        $data = WaInventoryItem::select([
            '*',
            DB::RAW('(select SUM(wa_stock_moves_C.qauntity) FROM wa_stock_moves_C where wa_stock_moves_C.wa_location_and_store_id = wa_inventory_items.store_location_id AND wa_stock_moves_C.stock_id_code = wa_inventory_items.stock_id_code) as quantity'),
        ])->where('store_c_deleted',0)->with(['getTaxesOfItem','pack_size'])->where('id',$request->id)->first();
        $view = '';
    ...
    public function getInventryItemDetails2(Request $request){
        ...
        $input_params_arr = explode("_",$request->id);


        //DB::enableQueryLog();

        $dataQry = DispatchLoadedProducts::with(['getSalesMan','getShift','getStoreLocation','getInventoryItem','getInventoryItem.getstockmoves','getInventoryItem.pack_size']);

        if($request->has('requistion_type') && $request->requistion_type=="invoice"){
            
            $dataQry=$dataQry->where(['store_location_id' => $input_params_arr[1], 'document_no' => $input_params_arr[0] , 'salesman_id' => $input_params_arr[2]]);

        }elseif($request->has('requistion_type') && $request->requistion_type=="loading_sheet"){
            $dataQry=$dataQry->where(['store_location_id' => $input_params_arr[1], 'shift_id' => $input_params_arr[0] , 'salesman_id' => $input_params_arr[2]]);
        }

        $data=$dataQry->where('balance_qty','>','0')->get();  
        ...
        if($data){
            return response(['status' => true,'message' =>'successfully','data'=>$data]);
        }
