...
class FamilyGroupController extends Controller
{
    ...
    {
        $this->model = 'family-groups';
        $this->title = 'Family Group';
        $this->pmodule = 'family-groups';
    ...
    public function index()
    {
        ...
        if(isset($permission[$pmodule.'___view']) || $permission == 'superadmin')
        {
            ...
            return view('admin.familygroups.index',compact('title','lists','model','breadcum','pmodule','permission'));
        } 
        else
        {
    ...
    public function create()
    {
        ...
        {
            $getParentList = $this->getParentList('forFamilyGroup');
            $getGlDetails = $this->getGLDetail();

            $title = 'Add '.$this->title;;
            $model = $this->model;
            $breadcum = [$this->title =>route($model.'.index'),'Add'=>''];
            return view('admin.familygroups.create',compact('title','model','breadcum','getParentList','getGlDetails'));
        }
        else
        {
            Session::flash('warning', 'Invalid Request');
            ...
            if(isset($permission[$this->pmodule.'___edit']) || $permission == 'superadmin')
            {
                ...
                if($row)
                {
                   $getParentList = $this->getParentList('forFamilyGroup');
                    ...
                    $model = $this->model;
                    return view('admin.familygroups.edit',compact('title','model','breadcum','row','getParentList','getGlDetails')); 
                }
                else
                {
                    Session::flash('warning', 'Invalid Request');
