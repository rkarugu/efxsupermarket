...
class BankReconciliationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            if (!can('reconcile', 'reconciliation')) {
            ...
            if ($request->intent == 'Process') {
                ...
                $end = Carbon::parse('2024-04-17')->endOfDay();
                ...
                $ebReader = new Xlsx();
                $ebReader->setReadDataOnly(true);

                $kcbReader = new Xlsx();
                $kcbReader->setReadDataOnly(true);
                ...
                if (count($dts) > 0) {
                    if (count($dts) > 1) {
                        ...
                        if ((float)$dt->amount == $recordAmount) {
                            ...
                            // Mark reconciled
                            WaDebtorTran::find($dt->id)->update(['reconciled' => true, 'bank_ref' => $lookupRef]);
                        } else {
...
//                }
                ...
                if (($lookupRef == null) || (strlen($lookupRef) < 5)) {
                    ...
                    if (count($dts) == 0) {
                        $dts = $debtorTrans->filter(function ($_dt) use ($altRef1) {
                            return str_contains($_dt->reference, $altRef1) || str_contains($altRef1, $_dt->reference);
                        });
                    }

                    if (count($dts) == 0) {
                        $dts = $debtorTrans->filter(function ($_dt) use ($altRef2) {
                            return str_contains($_dt->reference, $altRef2) || str_contains($altRef2, $_dt->reference);
                        });
                    }

                    if (count($dts) == 0) {
                        $dts = $debtorTrans->filter(function ($_dt) use ($altRef3) {
                            return str_contains($_dt->reference, $altRef3) || str_contains($altRef3, $_dt->reference);
                        });
                    }
                        ...
                        if (count($dts) > 1) {
                            ...
                            if (count($amountDts) > 1) {
                                ...
                                foreach ($amountDts as $dt) {
                                    $dt->bank_ref = $record[1] . "($recordAmount)";
                                    $this->doubles[] = $dt;
                                }
                                ...
                                if (count($amountDts) == 0) {
                                    ...
                                    // Mark reconciled
                                    WaDebtorTran::find($dt->id)->update(['reconciled' => true, 'bank_ref' => $bankRef]);
                                }
                            }
                        } else {
                            $dt = $dts->first();
...
//                $lookupRef = $refColumnContent[3] ?? null;
                    ...
                    if (count($dts) > 0) {
                        if (count($dts) > 1) {
                            ...
                            if (count($amountDts) > 1) {
                                ...
                                $this->notReconciled[] = $record;
                                foreach ($amountDts as $dt) {
                                    $dt->bank_ref = $bankRef . "($recordAmount)";
                                    $this->doubles[] = $dt;
                                }
    ...
    public function reconciled(Request $request)
    {
        ...
        return ExcelDownloadService::download('reconciled', $records, ['TRANS DATE', 'INPUT DATE', 'ROUTE', 'REFERENCE', 'DOC NO', 'AMOUNT', 'BANK_REF']);
    }

    public function systemUnreconciled(Request $request)
    {
        $records = DB::table('wa_debtor_trans')
            ->where('reconciled', false)
            ->where('document_no', 'like', '%RCT%')
        ...
        return ExcelDownloadService::download('system_unreconciled', $records, ['TRANS DATE', 'INPUT DATE', 'ROUTE', 'REFERENCE', 'DOC NO', 'AMOUNT']);
    }
