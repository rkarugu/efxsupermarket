...
class SlowMovingItemsReportController extends Controller
{
    protected $model;
    ...
    public function index()
    {
        ...
        $query = WaInventoryItem::query()
            ...
            ->where(DB::raw('IFNULL(total_sales,0)'), '<=', request()->sold ?? 5)
        ...
        if (request()->action) {
            ...
            if (request()->action == 'excel') {
                $view = view('admin.reports.export.slow_moving_items_report', [
                ...
                return Excel::download(new ExportViewToExcel($view), 'slow_moving_items_report_' . date('YmdHis') . '.xlsx');
            }
        }
        ...
        return view('admin.reports.slow_moving_item_report', [
            'model' => $this->model,
            'title' => $this->title,
