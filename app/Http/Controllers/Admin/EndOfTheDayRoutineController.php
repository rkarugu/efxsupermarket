...
use App\Model\WaEndOfTheDayRoutine;
use App\Model\Bill;
...
class EndOfTheDayRoutineController extends Controller
{
    ...
    {
        $this->model = 'end-of-the-day-routine';
        $this->title = 'End Of the day Routines';
        $this->pmodule = 'end-of-the-day-routine';
    } 
    ...
    public function index()
    {
        ...
        {
            $lists = WaEndOfTheDayRoutine::orderBy('id', 'desc')->get();
            $breadcum = [$title=>route($model.'.index'),'Listing'=>''];
            return view('admin.end_of_the_day_routine.index',compact('title','lists','model','breadcum','pmodule','permission'));
        }
        else
        {
    ...
    public function create(Request $request)
    {
        ...
        if(isset($permission[$this->pmodule.'___add']) || $permission == 'superadmin')
        {
	        ...
	        $checksamedate = WaEndOfTheDayRoutine::where('status','Open')->where('start_date',$date)->count();
			$bill = Bill::where('status','PENDING')->count();
			
			// if($bill > 0){
	        
	        //     Session::flash('warning', 'You can not open a new shift if there are any open bills.');
	        //     return redirect()->route($this->model.'.index');								
			
			// }else 
            if($checksamedate > 0){
	        
	            Session::flash('warning', 'Shift date is already open.');
	            return redirect()->route($this->model.'.index');				
			
			}else{
		    
		        WaEndOfTheDayRoutine::where('status','Open')->update(['status'=>'Closed','close_time'=>date('H:i:s')]);	        
				$createShift	 		 = new WaEndOfTheDayRoutine();
				$createShift->start_date = $date;
				$createShift->routine_no   = date('l-d/F/Y',strtotime($date));
				$createShift->open_time   = date('H:i:s');
				$createShift->save();
