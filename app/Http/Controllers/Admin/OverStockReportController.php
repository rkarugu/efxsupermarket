...
class OverStockReportController extends Controller
{
    protected $model;
    ...
    {
        $this->model = 'overstock-report';
        $this->title = 'Overstock Report';
    }

    public function index()
    {
        if (!can('overstock-report', 'inventory-reports')) {
        ...
        $returnsSub = WaInventoryLocationTransferItemReturn::query()
            ...
            // ->join('wa_inventory_location_transfer_items as items', 'items.id', '=', 'returns.wa_inventory_location_transfer_item_id', 'USE INDEX (wa_inventory_location_transfer_item_id)')
        ...
        $query = WaInventoryItem::query()
            ...
            ->whereColumn('stocks.quantity', '>', 'max_stocks.max_stock')
        ...
        if (request()->action) {
            ...
            if (request()->action == 'excel') {
                $view = view('admin.reports.export.overstock_report', [
                ...
                return Excel::download(new ExportViewToExcel($view), 'overstock_report_' . date('YmdHis') . '.xlsx');
            }
        }
        ...
        return view('admin.reports.overstock_qoh_max_stock_report', [
            'model' => $this->model,
            'title' => $this->title,
