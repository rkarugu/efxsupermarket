    ...
    <div>
        ...
        <div v-else>
            <div class="table-responsive" >
                <table class="table table-bordered table-hover" id="create_datatable_undelivered">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Route</th>
...
const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-undelivered/${props.branchId}/${props.startDate}`);      
        unfuelledRoutes.value = response.data

        $(document).ready(function() {
                $('#create_datatable_undelivered').DataTable({
                    'paging': true,
                    'lengthChange': true,
                    'searching': true,
                    'ordering': true,
