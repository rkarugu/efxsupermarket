    ...
    <div>
        ...
        <div v-else>
            <div class="table-responsive" >
                <table class="table table-bordered table-hover" id="create_datatable_unfuelled_routes">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Route</th>
...
const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-unfuelled-routes/${props.branchId}/${props.startDate}`);      
        console.log(response);
        unfuelledRoutes.value = response.data

        $(document).ready(function() {
                $('#create_datatable_unfuelled_routes').DataTable({
                    'paging': true,
                    'lengthChange': true,
                    'searching': true,
                    'ordering': true,
