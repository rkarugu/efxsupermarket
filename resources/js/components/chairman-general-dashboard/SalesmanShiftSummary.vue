                ...
                <table class="table table-bordered table-hover" id="create_datatable_10">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td style="text-align: center;">{{ totalCC }}</td>
                            <td style="text-align: center;">{{ totalMwO }}</td>
                            <td style="text-align: center;">{{ totalWO }}</td>
                            <td style="text-align: center;">{{ totalUnmet }}</td>
                            <td>
                                <div>
                                    <strong>{{ totalTonnage }} / {{ totalTonnageTarget }}</strong>
                                </div>
                            </td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td style="text-align: right;"><strong>{{ numberWithCommas(totalShiftTotal.toFixed(2)) }}</strong></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
...
const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-salesman-shift-summary/${props.branchId}/${props.startDate}`);
...
const totalCC = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + shift.shift_met_customers, 0));
const totalMwO = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + shift.met_with_order, 0));
const totalWO = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + shift.met_without_order, 0));
const totalUnmet = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + shift.unmet, 0));
const totalShiftTotal = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + parseFloat(shift.shift_total) || 0, 0));
const totalTonnage = computed(() => 
    salesmanShiftsData.value.reduce((sum, shift) => sum + parseFloat(shift.shift_tonnage) || 0, 0).toFixed(2)
);
const totalTonnageTarget = computed(() => salesmanShiftsData.value.reduce((sum, shift) => sum + shift.tonnage_target, 0));
</script>
