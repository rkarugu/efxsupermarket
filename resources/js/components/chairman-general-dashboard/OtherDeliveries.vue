    ...
    <div>
        ...
        <div v-else>
            <div class="table-responsive" >
                <table class="table table-bordered table-hover" id="create_datatable_other_deliveries">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Route</th>
...
const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-other-deliveries/${props.branchId}/${props.startDate}`);      
        unfuelledRoutes.value = response.data

        $(document).ready(function() {
                $('#create_datatable_other_deliveries').DataTable({
                    'paging': true,
                    'lengthChange': true,
                    'searching': true,
                    'ordering': true,
