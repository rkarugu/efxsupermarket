    ...
    <div>
        ...
        <div v-else>
            <div class="table-responsive" >
                <table class="table table-bordered table-hover" id="create_datatable_other_receivables">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Name</th>
                            ...
                            <th style="text-align: right;">Total Receivables</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(ySale, index) in ySalesData" :key="ySale.id">
                            ...
                            <td style="text-align: right;">{{ numberWithCommas(((ySale.returns + ySale.eazzy + ySale.vooma + ySale.mpesa + ySale.crc)).toFixed(2)) }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
...
const totalVariance = computed(() => ySalesData.value.reduce((sum, sale) => sum + ((sale.returns + sale.eazzy + sale.vooma + sale.mpesa + sale.crc)), 0));

const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-yesterday-sales/${props.branchId}/${props.startDate}/otherreceivables`);      
        ySalesData.value = response.data

        $(document).ready(function() {
                $('#create_datatable_other_receivables').DataTable({
                    'paging': true,
                    'lengthChange': true,
                    'searching': true,
                    'ordering': true,
