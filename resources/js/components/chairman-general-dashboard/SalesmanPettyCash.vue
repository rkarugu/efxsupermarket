    ...
    <div>
        ...
        <div v-else>
            <div class="table-responsive" >
                <table class="table table-bordered table-hover" id="create_datatable_salesman_petty_cash">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th >Reference</th>
                    ...
                    <tbody>
                        <tr v-for="(data, index) in salesmanPettyCashData" :key="data.shift_id">
                            <th>{{ index + 1 }}</th> 
...
const salesmanPettyCashData = ref([])
const totalTonnage = computed(() => salesmanPettyCashData.value.reduce((sum, data) => sum + data.tonnage, 0));
const totalSales = computed(() => salesmanPettyCashData.value.reduce((sum, data) => sum + data.gross_sales, 0));
const totalAmount = computed(() => salesmanPettyCashData.value.reduce((sum, data) => sum + data.amount, 0));

const fetchSalesmanShifts = async () => {
    loader.value = true;
    try {
        const response = await apiClient.get(`chairman-general-dashboard-salesman-petty-cash/${props.branchId}/${props.startDate}`);      
        salesmanPettyCashData.value = response.data

        $(document).ready(function() {
                $('#create_datatable_salesman_petty_cash').DataTable({
                    'paging': true,
                    'lengthChange': true,
                    'searching': true,
                    'ordering': true,
