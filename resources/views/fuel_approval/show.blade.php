...
@section('content')
    ...
    <section class="content" id="vue-mount">
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="box-header-flex">
                    <h3 class="box-title" v-cloak> Approval Details - @{{ verificationRecord.fueling_date }} </h3>

                    <div class="d-flex">
                        <a href="{{ url()->previous() }}" class="btn btn-primary ml-12"><i
            ...
            <div class="box-body">
                ...
                <ul class="nav nav-tabs" id="data-tabs">
                    ...
                    <li><a href="#uknown-resolved" data-toggle="tab">Unknown Resolved</a></li>
                    <li><a href="#uknown-pending" data-toggle="tab">Unknown Pending</a></li>
                </ul>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="verified-entries" v-cloak>
                        <div class="box-body">
                            <table class="table table-bordered table-hover data-tables" id="verified-entries-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th> <input type="checkbox" v-model="allSelected" id="verifyAllCheckbox"> </th>
                                    </tr>
                                </thead>
                                ...
                                <tbody v-cloak>
                                    <template v-for="(entry, index) in verifiedEntries" :key="index">
                                        <tr :style="{ backgroundColor: (entry.fuel_variance > 0) ? '#d0abab' : 'white' }">
                                            <th style="width: 3%;">
                                                <i class="fas fa-circle-plus" :id="`toggle-icon-${entry.entry_id}`"
                                                    style="cursor:pointer;" @click="toggleRow(entry)"></i>
                                            </th>
                                            <td> @{{ entry.fueling_date }}</td>
                                            <td> @{{ entry.shift_date }}</td>
                                            <td> @{{ entry.lpo_number }}</td>
                                            <td style="width: 15%;">
                                                <a :href="`/admin/delivery-schedules/${entry.delivery_id}`"
                                                    target="_blank">@{{ entry.route }}</a>
                                            </td>
                                            ...
                                            <td colspan="13">
                                                <div class="box">
                                                    <div class="box-body">
                                                        ...
                                                        <div class="d-flex"
                                                            style="width: 100%; border: 1px solid rgba(0, 0, 0, .125); margin-top: 20px;">
                                                            <div style="border-right: 1px solid rgba(0, 0, 0, .125); padding: 10px;"
                                                                class="d-flex justify-content-center align-items-center flex-grow-1">
                                                                <img :src="entry.dashboard_photo" alt="Dashboard Photo"
                                                            ...
                                                            <div class="d-flex justify-content-center align-items-center flex-grow-1"
                                                                style="padding: 10px;">
                                                                <img :src="entry.receipt_photo" alt="Receipt Photo"
                                                                    width="500" height="400"
                                                                    style="border-radius: 5px;">
                                                            </div>
                                                        </div>
                            ...
                            <div class="d-flex justify-content-end" style="margin-top: 10px;"
                                v-if="verifiedEntries.length > 0">
                                <button class="btn btn-success" @click="approve"><i
                                        class="fas fa-list-check btn-icon"></i>Approve Selected</button>
                            </div>
                        </div>
                    </div>

                    <div class="tab-pane" id="uknown-resolved">
                        <div class="box-body">
                            <table class="table table-hover table-bordered table-striped" id="create_datatable">
                                ...
                                <tbody>
                                    <tr v-for="(statement, index) in unknownPayments" :key="index">
                                        ...
                                        <div class="action-button-div">
                                            <input type="checkbox" v-model="statement.selected">
                                        </div>
                                    </td> --}}
                                    </tr>
                                </tbody>
                                ...
                                <tfoot v-cloak>
                                    <tr>
                                        ...
                                        <th style="text-align: right;"> @{{ getColumnTotal(unknownPayments, 'raw_fuel_total') }}</th>
                                        <th></th>
                                    </tr>
                                </tfoot>
                            </table>
                    ...
                    <div class="tab-pane" id="uknown-pending">
                        <div class="box-body">
                            <table class="table table-hover table-bordered table-striped" id="create_datatable">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <tr v-for="(statement, index) in unknownPending" :key="index">
                                        <th style="width: 3%;">@{{ index + 1 }}</th>
                                        <td>@{{ statement.timestamp }}</td>
                                        <td>@{{ statement.receipt_number }}</td>
                                        <td>@{{ statement.quantity }}</td>
                                ...
                                <tfoot v-cloak>
                                    <tr>
                                        <th colspan="7" style="text-align: right;"> TOTAL</th>
                                        <th style="text-align: right;"> @{{ getColumnTotal(unknownPending, 'raw_fuel_total') }}</th>
                                        <th></th>
                                    </tr>
                                </tfoot>
                            </table>
        ...
        <div class="modal fade" id="resolve-unknown-modal" tabindex="-1" role="dialog"
        ...
        <div class="modal fade" id="reset-unknown-modal" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
            <div class="modal-dialog" role="document">
                <div class="modal-content box box-primary">
                    <div class="box-header with-border">
    ...
    <script type="module">
        ...
        const app = createApp({
            ...
            mounted() {
                ...
                $('#verifyAllCheckbox').change(() => {
                    let checked = $('#verifyAllCheckbox').prop('checked');

                    if (checked) {
                        this.allSelected = true
                        this.verifiedEntries.forEach((entry) => {
                            $(`#select-${entry.entry_id}`).prop('checked', true)
                        })
                    } else {
                        this.allSelected = false
                        this.verifiedEntries.forEach((entry) => {
                            $(`#select-${entry.entry_id}`).prop('checked', false)
                        })
                    }
                });
            },
            ...
            methods: {
                ...
                fetchSummary() {
                    axios.get('{{ route('fuel-approval.summary') }}', {
                        params: {
                            record_id: this.verificationRecord.id
                        }
                    }).then(response => {
                ...
                fetchVerifiedEntries() {
                    axios.get('{{ route('fuel-approval.verified') }}', {
                        params: {
                            record_id: this.verificationRecord.id
                        }
                    }).then(response => {
                ...
                fetchUnknownPayments() {
                    axios.get('{{ route('fuel-approval.unknown') }}', {
                        params: {
                            record_id: this.verificationRecord.id
                        }
                    }).then(response => {
                ...
                fetchUnknownPending() {
                    axios.get('{{ route('fuel-approval.unknown-pending') }}', {
                        params: {
                            record_id: this.verificationRecord.id
                        }
                    }).then(response => {
                        this.unknownPending = response.data;
                ...
                toggleRow(entry) {
                    ...
                    this.verifiedEntries.forEach((_entry) => {
                        if (_entry.entry_id !== entry.entry_id) {
                            if (!($(`#child-row-${_entry.entry_id}`).hasClass('collapse'))) {
                                ...
                                $(`#toggle-icon-${_entry.entry_id}`).removeClass('fa-circle-minus')
                    ...
                    if (currentRow.hasClass('collapse')) {
                        ...
                        $(`#toggle-icon-${entry.entry_id}`).removeClass('fa-circle-plus').addClass(
                            'fa-circle-minus');
                    } else {
                        currentRow.addClass('collapse');
                        $(`#toggle-icon-${entry.entry_id}`).removeClass('fa-circle-minus').addClass(
                            'fa-circle-plus');
                    }
                },
                ...
                resetUnknown() {
                    $(".btn-loader").show();
                    axios.post('{{ route('fuel-verification.unknown.reset') }}', {
                        id: this.activeUnknown.id
                    }).then(response => {
                        $(".btn-loader").hide();
                ...
                approve() {
                    let selectedIds = [];
                    this.verifiedEntries.forEach((entry) => {
                        if ($(`#select-${entry.entry_id}`).prop('checked') == true) {
                            selectedIds.push(entry.entry_id);
                        }
                    })

                    $(".btn-loader").show();
                    axios.post('{{ route('fuel-approval.approve-verified') }}', {
                        ids: selectedIds
                    }).then(response => {
                        $(".btn-loader").hide();
                        this.toaster.successMessage('Entries approved successfully.');
