...
@section('content')
    @php
        ...
        $options = ['7 DAYS', '14 DAYS', '21 DAYS', '28 to 31 DAYS'];
    @endphp
    <script>
        window.user = {!! $user !!}
            window.branches = {!! $branches !!}
            window.route = {!! $route !!}
            window.selectedSupervisor = {!! $route->supervisors->pluck('user_id') !!}
    </script>
                    ...
                    <div class="tab-content">
                        ...
                        <div id="route-details" class="tab-pane" role="tabpanel" aria-labelledby="step-1">
                            <form method="post" class="form-horizontal" novalidate @submit.prevent="saveRouteDetails">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-10">
                                        <select name="branch_id" id="branch_id" class="form-control select2" required v-model="routeDetails.restaurant_id">
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    ...
                                    <div class="col-md-10">
                                        <select name="order_taking_days" id="order_taking_days" class="form-control select2" multiple>
                                            @foreach($weekDays as $index => $weekDay)
                                                <option value="{{ $index }}" @if(in_array($index, $route->order_taking_days)) selected @endif>{{ $weekDay }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    ...
                                    <div class="col-md-10">
                                        <select v-model="routeDetails.monthly_order_frequency" id="monthly_order_frequency" name="monthly_order_frequency" class="form-control select2" multiple>
                                            @foreach($options as $option)
                                                <option value="{{ $option }}" :selected="isSelected('{{ $option }}')">{{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    ...
                                    <div class="col-md-10">
                                        <select v-model="routeDetails.group" id="group" name="group" class="form-control">
                                            ...
                                            <option value="A" {{  $route->group == 'A' ? 'selected':'' }}>A</option>
                                            <option value="B" {{  $route->group == 'B' ? 'selected':'' }}>B</option>
                                            <option value="C" {{  $route->group == 'C' ? 'selected':'' }}>C</option>
                                            <option value="D" {{  $route->group == 'D' ? 'selected':'' }}>D</option>
                                            <option value="E" {{  $route->group == 'E' ? 'selected':'' }}>E</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="group" class="control-label col-md-2"> Supervisor</label>
                                    <div class="col-md-10">
                                        <select v-model="routeDetails.supervisor" id="supervisor" name="supervisor" class="form-control select2" multiple>
                                            <option value="">Select Option</option>
                                            <option v-for="item in supervisorOptions" :selected="isSupervisorSelected(item.id)":key="item.id" :value="item.id">@{{ item.name }}</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="group" class="control-label col-md-2"> Representative</label>
                                    <div class="col-md-10">
                                        <select v-model="routeDetails.representative" id="representative" name="representative" class="form-control select2">
                                            <option value="">Select Option</option>
                                            <option v-for="item in representativeOptions" :key="item.id" :value="item.id">@{{ item.name }}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div id="targets" class="tab-pane" role="tabpanel" aria-labelledby="step-2">
                            <form method="post" class="form-horizontal" novalidate @submit.prevent="saveRouteDetails" v-if="routeIsPhysical">
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="manual_fuel_estimate" class="control-label col-md-2"> Fuel Estimate</label>
                                    <div class="col-md-10">
                                        <input type="text" class="form-control" name="manual_fuel_estimate" id="manual_fuel_estimate" v-model="routeDetails.manual_fuel_estimate">
                                    </div>
                                </div>

                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="manual_distance_estimate" class="control-label col-md-2"> Distance Estimate</label>
                                    <div class="col-md-10">
                                        <input type="text" class="form-control" name="manual_distance_estimate" id="manual_distance_estimate" v-model="routeDetails.manual_distance_estimate">
                                    </div>
                                </div>

                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="manual_rate_estimate" class="control-label col-md-2"> Consumption Rate Estimate</label>
                                    <div class="col-md-10">
                                        <input type="text" class="form-control" name="manual_rate_estimate" id="manual_rate_estimate" v-model="routeDetails.manual_rate_estimate">
                                    </div>
                                </div>

                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="estimate_tonnage" class="control-label col-md-2"> Estimate Tonnage </label>
                                    <div class="col-md-10">
                                        <input type="text" class="form-control" name="estimate_tonnage" id="estimate_tonnage" v-model="routeDetails.estimate_tonnage">
                                    </div>
                                </div>
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    ...
                                    <div class="col-md-10">
                                        <input type="number" class="form-control" name="travel_expense" id="travel_expense"
                                               placeholder="0" min="0" step="0.01" v-model="routeDetails.travel_expense">
                                    </div>
                                </div>
                                <div class="form-group" v-if="routeIsPhysical">
                                ...
                                <div class="form-group" v-if="routeIsPhysical">
                                    <label for="estimated_shift_time" class="control-label col-md-2">Estimated Shift Time</label>
                                    <div class="col-md-10">
                                        <input type="number" class="form-control" name="estimated_shift_time" id="estimated_shift_time"
                                               placeholder="0" min="1"  v-model="routeDetails.estimated_shift_time">
                                    </div>
                                </div>
                                ...
                                <div class="box-footer">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <button class="btn btn-primary" @click="saveRouteDetails"> Update Route</button>
                                    </div>
                                </div>
                            </form>
...
@section('uniquepagescript')
    ...
    <script src="https://maps.googleapis.com/maps/api/js?loading=async&libraries=places&key={{ $googleMapsApiKey }}"></script>
        ...
        const app = createApp({
            data() {
                return {
                   ...
                   startMonthlyFrequencyArray: [],
                   endMonthlyFrequencyArray: [],
                    options: ['7 DAYS', '14 DAYS', '21 DAYS', '28 to 31 DAYS'],
                    option: null,
                    routeDetails: {
                    ...
                    supervisorOptions: [],
                    selectedSupervisor: window.selectedSupervisor
                }
            },
            ...
            mounted() {
                ...
                this.routeDetails = this.route
                this.routeDetails.representative = this.route.current_representative?.user_id
                this.routeDetails.supervisor = window.selectedSupervisor
                // let orderTakingDays = []
                // this.routeDetails.order_taking_days.forEach(value => {
                //     orderTakingDays.push(parseInt(value));
                // })
                // this.routeDetails.order_taking_days = orderTakingDays
                // $("#order_taking_days").val(orderTakingDays);

                // let startMonthlyFrequencyArray = [];
                // let endMonthlyFrequencyArray = [];

                // startMonthlyFrequencyArray = this.routeDetails.start_monthly_order_frequency.split(',');
                // endMonthlyFrequencyArray = this.routeDetails.end_monthly_order_frequency.split(',');

                // const mergedOrderFrequencies = [];
                // for (let i = 0; i < startMonthlyFrequencyArray.length; i++) {
                //     mergedOrderFrequencies.push([startMonthlyFrequencyArray[i], endMonthlyFrequencyArray[i]]);
                // }

                // startMonthlyFrequencyArray = [];
                // endMonthlyFrequencyArray = [];

                // mergedOrderFrequencies.forEach(pair => {
                //     startMonthlyFrequencyArray.push(pair[0]);
                //     endMonthlyFrequencyArray.push(pair[1]);
                // });

                this.startMonthlyFrequencyArray = this.routeDetails.start_monthly_order_frequency ? this.routeDetails.start_monthly_order_frequency.split(',') : null;
                this.endMonthlyFrequencyArray = this.routeDetails.end_monthly_order_frequency ? this.routeDetails.end_monthly_order_frequency.split(',') : null;
                
                this.routeDetails.loading_latitude = this.route.start_lat
                this.routeDetails.loading_longitude = this.route.start_lng
                this.route.centers.forEach((center, index) => {
                ...
                $("#branch_id").change(() => {
                    ...
                    this.fetchUsersData(this.routeDetails.restaurant_id);
                });
                ...
                $("#supervisor").change(() => {
                    this.routeDetails.supervisor = $("#supervisor").val();
                })

                $("#representative").change(() => {
                    this.routeDetails.representative = $("#representative").val();
                })
                this.fetchUsersInitialData();
            },
            ...
            methods: {
                ...
                isSelected(optionValue) {
                    let newValue = '';
                    if (this.startMonthlyFrequencyArray && this.endMonthlyFrequencyArray) {
                        newValue = this.startMonthlyFrequencyArray.includes(optionValue.substring(0, optionValue.indexOf(' '))) ||
                        this.endMonthlyFrequencyArray.includes(optionValue.substring(0, optionValue.indexOf(' ')))
                    }
                    return (newValue);
                },

                isSupervisorSelected(optionValue) {
                    
                    if (this.selectedSupervisor) {console.log(optionValue);
                        // Assuming `selectedSupervisor` is an array of supervisor IDs
                        return this.selectedSupervisor.includes(optionValue);
                    }
                    return false;
                },
                ...
                saveRouteDetails(e) {
                    ...
                    // if (!this.routeDetails.representative) {
                    //     return this.toaster.errorMessage('Representative is required')
                    // }
                    ...
                    axios.post('/api/routes/update', this.routeDetails).then(res => {
                        $("#loader").css('display', 'none');
                        this.toaster.successMessage('Route updated successfully')
                ...
                saveCenters() {
                    ...
                    let payload = {
                        route_id: this.routeDetails.id,
                        centers: this.centers
                    }
                    console.log(payload);
                    
                    axios.post('/api/delivery-centers/update', {payload: JSON.stringify(payload)}).then(res => {
                        $("#loader").css('display', 'none');
                        this.toaster.successMessage('Centers updated successfully')

                        return window.location.assign('/admin/manage-routes')
                    }).catch(error => {
                        $("#loader").css('display', 'none');
                        this.toaster.errorMessage(error.response?.data?.message)
                    })
                ...
                fetchUsersData(branch) {
                    axios.get(`/api/route-get-users-branch/${branch}`)
                        .then(response => {
                            this.representativeOptions = response.data.representatives; 
                            this.supervisorOptions = response.data.supervisors; 
                        })
                        .catch(error => {
                        console.error(error);
                        });
                },
                fetchUsersInitialData(){
                    this.fetchUsersData(this.routeDetails.restaurant_id)
                }
            },
        })
