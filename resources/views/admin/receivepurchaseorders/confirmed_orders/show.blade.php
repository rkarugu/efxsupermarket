                    ...
                    <div class = "col-sm-6">
                        ...
                        <div class="form-group">
                            <div class = "row">
                                ...
                                <div class="col-sm-7">
                                    {!! Form::text('invoice_control_amount', null, [
                                        'class' => 'form-control',
                                        'disabled' => true,
                                        'id' => 'invoice_control_amount',
                                    ]) !!}
                                </div>
                            </div>
                        </div>
                                                ...
                                                @foreach ($files as $key => $val)
                                                    @if ($key != 'from_portal')
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a data-toggle="document" data-title="Documents"
                                                                    @if (isset($files['from_portal'])) data-url="{{ $val }}"  @else data-url="{{ asset('uploads/purchases_docs/' . @$val) }}" @endif>
                        ...
                        <tbody>
                            @if ($row->child_items && count($row->child_items) > 0)
                                    ...
                                    @if ($getRelatedItem->delivered_quantity > 0)
                                        ...
                                        <tr>
                                            ...
                                            {{-- <td class="align_float_right">
                                                ...
                                                <td class="align_float_right"
                                                    id="discount_amount_{{ $getRelatedItem->id }}">0.00</td> --}}
                                ...
                                <tr id = "last_total_row">
                                    <td colspan="4">
                                        @if ($row->status == 'Confirmed' && can('confirm', 'confirmed-receive-purchase-order'))
                                            <input type="submit" class="btn btn-success" value="Process"
                                                name="approval_status" id="process_button">
                                            {{-- <input type="submit" class="btn btn-success" value="Reject"
                                                    name="approval_status" id="reject_button"> --}}
                                        @endif
                                    </td>
                                    <td colspan="6" class="text-right"></td>
                                    <td class="align_float_right" id= "main_all_total"></td>
                                </tr>
                                <tr id = "last_total_row">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#submit_form').on('submit', function(e) {
                ...
                var originalButtonText = clickedButton.val();
                clickedButton.val('Processing...');

                $('input[type="submit"]').prop('disabled', true);

                var formData = new FormData(this);
                formData.append('approval_status', originalButtonText);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        form.successMessage('GRN Processed Successfully.')
                        setTimeout(() => {
                        ...
                        if (xhr?.responseJSON?.errors) {
                            for (let key in xhr.responseJSON.errors) {
                                if (xhr.responseJSON.errors.hasOwnProperty(key)) {
                                    errorMessage +=
                                        `<span style="color: red;">${xhr.responseJSON.errors[key].join('<br>')}</span><br>`;
                                }
                            }
                    ...
                    complete: function() {
                        ...
                        clickedButton.val(originalButtonText);
                    }
                });
            });
        });
