...
<br>
<div style="text-align: center;" class="underline"><b>SALESMAN INVOICE</b></div>
<div>Delivery Loc: {{ $row->getBranch->name}} - {{ $row->getDepartment->department_name }}</div>
<div>Invoice No: {{ $row->requisition_no}} <div style="text-align: right;">Order Date: {{ $row->requisition_date}}</div></div><br>
