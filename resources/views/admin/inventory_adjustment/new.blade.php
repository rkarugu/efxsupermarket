                            ...
                            <div id = "requisitionitemtable" name="item_id[0]">
                                <table class="table table-bordered table-hover" id="mainItemTable">
                                    ...
                                    <tr>
                                      ...
                                      <th  style="width: 12%">Adj To Store Loc.</th>
                                      <th  style="width: 10%">Curr Qty</th>
...
<script src="{{asset('assets/admin/bower_components/select2/dist/js/select2.full.min.js')}}"></script>
  ...
  function getAndUpdateItemAvailableQuantity(input_obj){
      ...
      if(location_id){
          stock_id_code = $(input_obj).parents('tr').find('.testIn').val();
          jQuery.ajax({
              ...
              success: function (response) {
                $(input_obj).parents('tr').find('.bal_stock').val(response['available_quantity']);
              }
          });
      }
      else{
        $(input_obj).parents('tr').find('.bal_stock').val(0);
      }
...
<script src="{{asset('assets/admin/dist/bootstrap-datepicker.js')}}"></script>
  ...
  $(document).on('keyup','.testIn',function(e){ var vale = $(this).val();
      ...
      $.ajax({
        type: "GET",
        url: "{{route('inventory-item-adjustment.inventoryItems')}}",
        data: {
  ...
  $(document).click(function(e){
    ...
    function fetchInventoryDetails(varia){
        ...
        var furtherCall = true;
            ...
            if(furtherCall == true){
                $.ajax({
                    type: "GET",
                    url: "{{route('inventory-item-adjustment.getInventryItemDetails')}}",
                    data: {
                        'id':$this.data('id')
                    },
                    success: function (response) {
                        $('.load_location').select2('destroy');
    ...
    var load_location = function(){
            $(".load_location").select2(
            {
                placeholder:'Select Payment Account',
                ajax: {
                    url: '{{route('inventory-item-adjustment.location_list')}}',
                    dataType: 'json',
                    type: "GET",
                    delay: 250,
