...
@section('content')
<form class="validate form-horizontal" role="form" method="POST" action="{{ route('stock-processing.return.store') }}"
...
<script>
        ...
        $.ajax({
            ...
            success:function(out){
                ...
                if(out.result === 1) {
                    ...
                    if(out.id)
                    {
                        location.href = '/admin/stock-processing/return/file/PDF/'+out.id;
                        setTimeout(
                        function() 
                        {                        
                            location.href = `{{route('stock-processing.return')}}`;
                        }, 3000);                    
                    } else{
                        location.href = `{{route('stock-processing.return')}}`;
                    }
                    ...
                    if (err?.responseJSON?.errors) {
                        ...
                        for (let key in err.responseJSON.errors) {
                            if (err.responseJSON.errors.hasOwnProperty(key)) {
                                // errorMessage = err.responseJSON.errors
                                errorMessageSwal += err.responseJSON.errors[key].join('<br>') + '<br>';
                            }
                        }
                    } else if (err?.responseJSON?.error) {
                        errorMessageSwal = err.responseJSON.error
                    }
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        html: errorMessageSwal,
                    });
            },
                complete: function() {
                    getDataBtn.prop('disabled', false).text(originalGetDataText);
    ...
    $(document).click(function(e){
        function fetchDisabledDates(id) {
            var url = "{{ route('stock-dates',[':id','type'=>'excess']) }}";
