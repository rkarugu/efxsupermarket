        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="box-header-flex">
                    <h3 class="box-title" style="font-weight:500 !important;"> Stock Pending Entries </h3>
                    <div class="d-flex">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <div class="form-group">
                    @if (can('print', 'stock-pending-entries'))
...
<script>
    $(function() {
        ...
        $('#generatePDFBtn').on('click',function(){
            location.href=`/admin/stock-pending-entries?type=pdf&status=`+$("#status").val()+`&channel=`+$("#channel").val()+`&branch=`+$("#branch").val()+`&route=`+$("#route").val()+`&start_date=`+$("#start_date").val()+`&end_date=`+$("#end_date").val();
        });
        let table = $("#debtorDataTable").DataTable({
            ...
            ajax: {
                url: '{!! route('stock-pending-entries.index') !!}',
                data: function(data) {
                        ...
                        render: function (data, type, row, meta) {
                            if (type === 'display') {
                                var actions = '<div class="d-flex">';
                                    @if (can('restore', 'stock-pending-entries'))
                                        actions += `<a href="javascript:void(0)" onclick="restore(`+row.id+`)" title="Restore"><i class="fas fa-window-restore text-success"></i></a>`;
                                    @endif
                                    @if (can('expunge', 'stock-pending-entries'))
                                        actions += `<a href="javascript:void(0)" onclick="expunge(`+row.id+`)" title="Expunge" style="margin-left:10px;"><i class="fas fa-trash text-danger"></i></a>`;
                                    @endif
                                    actions +='</div>';
                                return actions;
                            }
        ...
        $('#confirmRestoreBtn').on('click', function (e) {
            ...
            $.ajax({
                url: "{{route('stock-pending-entries.restore')}}", 
                type: 'POST',
                headers: {
        ...
        $('#confirmExpungeBtn').on('click', function (e) {
            e.preventDefault();           
            var postData = { 
                variation: $(this).data('id')
            };
            ...
            $.ajax({
                url: "{{route('stock-pending-entries.expunge')}}", 
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') 
                },
