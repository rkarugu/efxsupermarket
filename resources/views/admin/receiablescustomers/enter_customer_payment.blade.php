...
<section class="content">    
    <div class="box box-primary">
        <div class="box-header with-border"> <b>
                   Receive Payment From {{ ucfirst($customer->customer_name) }} on {{ date('d/m/Y')}} 
                             </b></div>

         @include('message')
        <form class="validate form-horizontal"  role="form" method="POST" action="{{ route($model.'.post-customer-payment',$customer->slug) }}" enctype = "multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="box-body">
             ...
             <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        {!!Form::select('payment_type_id', $cash_methods, null, ['placeholder'=>'Select payment type', 'class' => 'form-control select2' , 'required'=>true ])!!}
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-sm-10">
                        <table class="table table-responsive bg bg-danger">
                            <thead class="">
                                ...
                                <tbody>
                                    @foreach($customertrans as $k=> $supptran)
                                        <tr>
                                            <td>{{$supptran->document_no}}</td>
                                            <td>{{$supptran->amount}}</td>
                                            <td>{{$supptran->allocated_amount}}</td>
                                            @php
                                             $tobeallocatedamount = $supptran->amount - $supptran->allocated_amount;
                                            @endphp
...
<script>
    ...
    @if(session()->has('debtrpayment'))
     $(document).ready(function () {
        printBill('{{session()->get('debtrpayment')}}');
     });
     @php 
     session()->forget('debtrpayment');
     @endphp
    @endif
</script>
@endsection
