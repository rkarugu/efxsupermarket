         ...
         {!! Form::model($row, ['method' => 'PATCH','route' => [$model.'.update', $row->slug],'class'=>'validate','enctype'=>'multipart/form-data']) !!}
                ...
                <div class="col-sm-10">
                    <select name="chief_storekeeper" id="chief_storekeeper" class="select2" required>
                        ...
                        @foreach ($users as $user)
                            <option value="{{$user->id}}" @if ($user->id == $row->chief_storekeeper) selected @endif>{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="form-group" >
                ...
                <div class="col-sm-10">
                    ...
                    <input type="checkbox" name="is_display" id="is_display" style="margin-top: 8px; margin-bottom: 8px;" {{ $row->is_display ? 'checked' : '' }}>
