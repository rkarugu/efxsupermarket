        ...
        <div class="box box-primary">
            ...
            <div class="box-header with-border">
                <div class="d-flex justify-content-between">
                    <h3 class="box-title">DOZENS  Report</h3>
                        ...
                        @if(isset($data))
                            ...
                            @foreach ($data as $record)
                                <tr>
                                    ...
                                    <td>{{ number_format(($record->dzn_target * $record->total_order_taking_days), 2) }}</td>
                                    <td>
                                        @if ($filterfrequency != null)
                                            {{ manageAmountFormat(floatval($record->dzns) * floatval($filterfrequency)) }}
                                        @else
                                            {{ manageAmountFormat($record->dzns) }}
                                        @endif
                                    </td>
                                    <td>
                                        <a target="blank"
                                           href="{{ route('sales-and-receivables-reports.group-filter-route-item-report', ['route_id' => $record->route_id, 'frequency_filter' => request()->frequency_filter, 'ctns_dzns' => 'dzns', 'start_date' => $start_date, 'end_date' => $end_date]) }}">
                                            @if ($filterfrequency != null)
                                                {{ $record->dzns_percentage * floatval($filterfrequency) }}
                                            @else
                                                {{ number_format($record->dzns_percentage) }}
                                            @endif
                                        </a>
                                    </td>
                                </tr>
