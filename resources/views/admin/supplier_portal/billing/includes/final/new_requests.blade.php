<div style="margin-top: 30px">
    <div class="table-responsive">
        ...
        <table class="table table-bordered" id="new_bank_slips_table">
            ...
            <tbody>
                @foreach ($billing_slip_approvals_data->filter(fn($record) => $record->status === 'Approved' && $record->approve_status === 'Initial') as $record)
                    <tr>
                        <td>
                            <input type="checkbox" class="select-item" value="{{ $record->id }}"
                                data-billing-id="{{ $record->billing_bank_payment_id }}">
                        </td>
