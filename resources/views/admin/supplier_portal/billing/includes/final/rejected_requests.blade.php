<div style="margin-top: 30px">
    <div class="table-responsive">
        <table class="table table-bordered" id="rejected_bank_slips_table">
            <thead>
                <tr>
                    <th>SUPPLIER</th>
                    <th>AMOUNT</th>
            ...
            <tbody>
                @foreach ($billing_slip_approvals_data->filter(fn($record) => $record->approve_status === 'Final') as $record)
                    @foreach ($record->billingsupplierdocumentprocesslogs->filter(fn($item) => $item->updated_status === 'Rejected'
                    && $item->updated_approve_status === 'Final' && $item->stage === 'Final') as $item)
                        <tr>
                                    ...
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="filesModalLabel-{{ $record->id }}">Bank
                                                    Slips
                                                </h5>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row">
                                                    @foreach ($record->billingsupplierdocumentprocessfiles as $file)
    ...
    <div class="form-group mt-3" id="action-buttons" style="display:none;">
