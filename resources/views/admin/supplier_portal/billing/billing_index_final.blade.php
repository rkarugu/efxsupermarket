            ...
            <div class="box-body">
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="new_requests_tab">
                        @include('admin.supplier_portal.billing.includes.final.new_requests')
                    </div>
                    <div class="tab-pane" id="approved_requests_tab">
                        @include('admin.supplier_portal.billing.includes.final.approved_requests')
                    </div>
                    <div class="tab-pane" id="rejected_requests_tab">
                        @include('admin.supplier_portal.billing.includes.final.rejected_requests')
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function handleButtonClick(status, clickedButton, otherButton) {
                var selectedIds = [];
                var billingBankPaymentIds = [];
                $('.select-item:checked').each(function() {
                    selectedIds.push($(this).val());
                    billingBankPaymentIds.push($(this).data('billing-id'));
                });
                ...
                if (selectedIds.length > 0) {
                    ...
                    $.ajax({
                        url: "{{ route('update_billing_slip_status_final.update') }}",
                        method: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
                            ids: selectedIds,
                            billing_bank_payment_id: billingBankPaymentIds,
                            status: status
                        },
