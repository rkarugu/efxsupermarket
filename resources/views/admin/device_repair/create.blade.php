        ...
        <div class="box-body">
            ...
            <form class="validate form-horizontal"  role="form" method="POST" action="{{ route($model.'.store') }}">
                        ...
                        <div class="col-sm-10">
                            <select name="device" id="device" class="form-control select2">
                                ...
                                @foreach ($devices as $device)
                                    <option value="{{$device['id']}}">{{ $device['device_no'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group" id="current_holder_div">
                        <label for="current_holder" class="col-sm-2 control-label">Current Device Holder</label>
                        <div class="col-sm-10">
                            <input class="form-control" id="current_holder" type="text" disabled>  
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="repair_cost" class="col-sm-2 control-label">Repair Cost</label>
                        <div class="col-sm-10">
                            <input class="form-control" name="repair_cost" id="repair_cost" type="number">  
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select name="charge_to" id="charge_to" class="form-control select2">
                                ...
                                <option value="{{ $settings['COMPANY_NAME'] }}">{{ $settings['COMPANY_NAME'] }}</option>
                ...
                <div class="box-footer">
                    <button type="submit" class="btn btn-primary addDeviceRepair">Submit</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        $('#device').on('change', function () {
            ...
            if ($(this).val()) {
                $.ajax({
                    url: '/admin/get-device-holder/'+$(this).val(),
                    ...
                    success:function(response){
                        if (response && response.current_holder) {
                            $('#current_holder').val(response.current_holder)
                            $('#current_holder_div').show(); 
                        } else {
                            $('#current_holder_div').hide();
                        } 
                    },
    ...
    $(document).on('click','.addDeviceRepair',function(e){
        e.preventDefault();

        let processDataBtn = $('.addDeviceRepair');
