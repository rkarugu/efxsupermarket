            ...
            <form>
                ...
                <div class="form-group">
                    <div class="col-sm-2">
                        ...
                        <input type="date" name="from" id="from" class="form-control" value="{{ request()->from }}" required>
                        {{-- {!! Form::text('from',request()->from, ['maxlength'=>'255','placeholder' => 'Start Date', 'required'=>true, 'class'=>'form-control datepicker','readonly'=>true]) !!}   --}}
                    </div>
                    <div class="col-sm-2">
                        <label for="to">End Date</label>
                        <input type="date" name="to" id="to" class="form-control" value="{{ request()->to }}" required>
                        {{-- {!! Form::text('to',request()->to, ['maxlength'=>'255','placeholder' => 'End Date', 'required'=>true, 'class'=>'form-control datepicker','readonly'=>true]) !!}   --}}
                    </div>
                    <div class="col-sm-3">
                    ...
                    @if (count($bankTypes))
                        <div class="col-sm-3">
                            ...
                            <select name="type" id="type" class="mlselect">
                                <option value="" selected>Select All</option>
                                @foreach ($bankTypes as $item)
                                    <option value="{{$item->code}}" {{request()->type == $item->code ? 'selected' : NULL}}>{{$item->description}}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                    @if (count($narrationTypeU))
                        <div class="col-sm-2">
                            <label for="narrative_type">Narrative Type</label>
                            <select name="narrative_type" id="narrative_type" class="mlselect">
                                <option value="" selected>Select All</option>
                                @foreach ($narrationTypeU as $item)
                                    <option value="{{$item}}" {{request()->narrative_type == $item ? 'selected' : NULL}}>{{$item}}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
                    ...
                    <div class="col-sm-12">
                            ...
                            <div class="col-md-12 no-padding-h">
                                <table class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <td style="font-weight: bold;text-align:right" colspan="7">Opening Balance</td>
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="details detailsParent tddHead"  > 
                                          ...
                                          @endphp
                                             <td>
                                                {{-- {!! isset($number_series_list[$list['type_number']])?$number_series_list[$list['type_number']] : '' !!} --}}
                                                {!! $list['type_number'] ?? '' !!}
                                            </td>
                                            ...
                                            <td>{!! $list['narration'] !!}</td>
                                             ...
                                             <td style="text-align: right">{!! $list['amount'] > 0 ? @manageAmountFormat($list['amount']) : '-' !!}</td>
                                             <td style="text-align: right">{!! $list['amount'] < 0 ? @manageAmountFormat(abs($list['amount'])) : '-' !!}</td>
                                    ...
                                    <tfoot>
                                      <tr>
                                        <td style="font-weight: bold;text-align:right;border-top: 1px solid #000;border-bottom: 1px solid #000;" colspan="4"></td>
...
$(document).ready(function() {
    $('#slug').on('change', function() {
        ...
        $('#narrative_type').val(null).trigger('change');
    });
