...
@section('content')
<form method="POST" action="{{ route('store-c-requisitions.store2')}}" accept-charset="UTF-8" class="" onsubmit="return false;" enctype="multipart/form-data" >
            ...
            <div class = "row">
             ...
             <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="hidden" name="idds" value="" id="idds">
        ...
        <!-- Small boxes (Stat box) -->
                    <div class="box box-primary">
                        ...
                        <div class="col-md-4 no-padding-h ">
                            <select class="form-control select_requistion_type mlselec6t" required >
                                <option selected value="" >Select Requistion Type</option>
                                <option value="loading_sheet">Loading Sheet</option>
                                <option value="invoice" >Invoice</option>
                            </select>
                        </div>        
                          
                        <div class="col-md-4 no-padding-h" id="loading_sheet_div"  >
                            <select class="form-control select_requistion   mlselec6t" >
                             <option value="" required>{{'Select Requistion'}}</option>
                              <?php foreach ($dispatchLoadingSheetData as  $value) {?>
                                <option value="{{@$value->shift_id}}_{{!is_null(@$value->store_location_id)?@$value->store_location_id:''}}_{{@$value->salesman_id}}" name="ids">{{@$value->getShift->shift_id}},{{isset($value->getStoreLocation)?@$value->getStoreLocation->location_name:'N/A'}},{{@$value->getSalesMan->name}}</option>
                              <?php } ?>
                            </select>
                        </div>

                        <div class="col-md-4 no-padding-h " id="invoice_div">
                            <select class="form-control select_requistion mlselec6t" >
                             <option value="" required>{{'Select Requistion'}}</option>
                              <?php foreach ($dispatchInvoiceData as  $value) {?>
                                <option value="{{@$value->document_no}}_{{!is_null(@$value->store_location_id)?@$value->store_location_id:''}}_{{@$value->salesman_id}}" name="ids">{{@$value->document_no}},{{isset($value->getStoreLocation)?@$value->getStoreLocation->location_name:'N/A'}},{{@$value->getSalesMan->name}}</option>
                              <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-12 no-padding-h ">

                          <!-- <h3 class="box-title">Requistion</h3> -->
...
<script type="text/javascript">
    ...
    $('#loading_sheet_div').hide();  
    $('#invoice_div').hide();  
    $('.select_requistion_type').on('change',function(){
      // alert();
        var requistion_type=$(this).val();
        if(requistion_type=="invoice"){
            $('#loading_sheet_div').hide();  
            $('#invoice_div').show(); 
        }else if(requistion_type=="loading_sheet"){
            $('#loading_sheet_div').show();  
            $('#invoice_div').hide(); 
        }else{
            $('#loading_sheet_div').hide();  
            $('#invoice_div').hide();      
        }
        $(".select_requistion option:selected").prop("selected", false)
...
<script src="{{asset('assets/admin/dist/bootstrap-datepicker.js')}}"></script>
  ...
  $(document).click(function(e){
    ...
    $(document).on('click','.addNewrow',function(){
      ...
      $( ".select_requistion" ).on( "change", function() {
        var id = $(this).val();
       // $('#mainItemTable tbody').append('');
        $('#idds').val(id);
          $.ajax({
          type: "GET",
          url: "{{route('store-c-requisitions.getInventryItemDetails2')}}",
          data: {
            'id':id,
            'requistion_type':$('.select_requistion_type').val(),
          },
          success: function (response) {
            var html =''; 
            $.each(response.data,(function(index,reque){


              html += '<tr><td><input type="hidden" name="item_id['+reque.get_inventory_item.id+']" class="itemid" value="'+reque.get_inventory_item.id+'"><input type="text" class="form-control" value="'+reque.get_inventory_item.stock_id_code+'" readonly><div class="textData" style="width: 100%;position: relative;z-index: 99;"></div></td>'
                                    +'<td> <input class="form-control" type="text" value="'+reque.get_inventory_item.description+'" readonly name="item_description['+reque.get_inventory_item.id+']" > </td>'
                                    +'<td>0.00</td>'
                                    +'<td> <input type="hidden" readonly name="item_unit['+reque.get_inventory_item.id+']" value="'+reque.get_inventory_item.pack_size.id+'">  '+reque.get_inventory_item.pack_size.title+'</td>'
                                    +'<td> <input type="text" readonly class="form-control"  name="item_quantity['+reque.get_inventory_item.id+']" value="'+reque.balance_qty+'" ></td>'
                                    +'<td> <input type="hidden" readonly name="store_location_id['+reque.get_inventory_item.id+']" value="'+reque.get_store_location.id+'"> '+reque.get_store_location.location_name+'</td>'
                                    +'</tr>';
                                    //+'<input type="hidden" value="'+reque.store_location_id+'" name="to_store_id"'
            }));

            // html += '<tr><td><input type="hidden" value="'+response.data[0].store_location_id+'" name="to_store_id"><td></tr>';
            $('#mainItemTable tbody').html(html);
