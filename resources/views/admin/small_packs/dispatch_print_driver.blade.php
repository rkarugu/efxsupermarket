...
<div id="receipt-main">
    <div id="receipt-header" >
        <h3 style="margin: 10px; padding: 0; font-size: 45px;"> {{ $settings['COMPANY_NAME'] }} </h3>
    ...
    <div style="margin-top: 20px; text-align: center;">
        <h3 style="margin: 0; padding: 0; font-size: 50px;"> SMALL PACK DISPATCH </h3>
        <span class="normal" style="font-size: 45px; font-weight: 300">---{{ $dispatchInfo['document_no'] }} ---</span>
    </div>
    ...
    <div style="margin-top: 20px; text-align: center" class="customer-details">
        ...
        @if ($dispatchInfo['print_count'] > 0)
            <span style="font-size:35px !important">REPRINT {{$dispatchInfo['print_count']+1}}</span>
        @endif
        <br>
    </div>
    <br>
    ...
    <table style="margin-top:20px;!important;">
        <tr class="heading" style="font-size: 40px">
    ...
    <table class="table">
        ...
        @foreach ($item['items'] as $key => $item)
            <tr class="" style="width: 100%">
                <td style="font-size: 34px">{{ $loop -> iteration }}.</td>
                <td style="font-size: 34px">
                    {{@$item['title']}}
                </td>
                <td style="font-size: 34px">{{@$item['totalQty']}}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
