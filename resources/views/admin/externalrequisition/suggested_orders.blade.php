            ...
            <form action="" method="get" role="form">
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <select name="store" class="form-control" id="store" @if(getLoggeduserProfile()->role_id != 1) disabled @endif>
                                <option value="" selected>Show All</option>
                                @foreach($stores as $key =>  $store)
                                    <option value="{{ $store['id'] }}" 
                                    @if((request()->store == $store['id']) || (getLoggeduserProfile()->wa_location_and_store_id == $store['id']) && getLoggeduserProfile()->role_id != 1) selected @endif>{{ $store['name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            <label for="" style="color:white;"> Actions</label>
                            <div>
