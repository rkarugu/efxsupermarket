...
<br>
<div style="text-align: center;" class="underline"><b>PURCHASE REQUISITION</b></div>
<div>Delivery Loc: {{ $row->getBranch->name}} - {{ $row->getDepartment->department_name ?? '' }}</div>
<div>Order: {{ $row->purchase_no}} <div style="text-align: right;">Order Date: {{ $row->requisition_date}}</div></div><br>
...
<tr>
	...
	<td width = "30%">{{ @$getRelatedItem->getInventoryItemDetail->pack_size->title }}</td>
