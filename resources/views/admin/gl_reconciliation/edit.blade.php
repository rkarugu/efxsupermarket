        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="box-header-flex">
                    <h3 class="box-title"> Edit GL Reconcile </h3>
                    <div class="d-flex">
            ...
            <div class="box-body">
                ...
                <form id="fetchPaymentForm" action="{{ route('gl-reconciliation.update',$data->id) }}" method="post">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                              ...
                              <input type="date" name="ending_date" id="ending_date" class="form-control" value="{{$data->end_date}}" required>
                            </div>
                        </div>
                        <div class="form-group col-sm-4 text-right">
                            @php
                                $bal = abs($data->beginning_balance) - abs($data->extras->sum('amount'));
                                $diff =  $data->ending_balance - $bal;
                            @endphp
                            <h2 style="margin-bottom: 0px;">Ksh. <span id="difference">{{ manageAmountFormat($diff) }}</span></h2>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                              ...
                              <span class="form-control balance">{{$data->beginning_balance}}</span>                    
                              <input type="hidden" value="{{$data->beginning_balance}}" class="form-control compute-input" name="balance" id="balance" readonly>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="">Ending Balance</label>      
                                <input type="text"  name="ending_balance" onkeyup="addThousandSeparator(this)" class="form-control compute-input" value="{{$data->ending_balance}}" id="ending_balance" required>
                            </div>
                        </div>
                    </div>
                    <hr>
                        ...
                        @if ($item->type=='Expense')
                            <div class="row">
                                <div class="col-sm-4">
                                    ...
                                    <input type="date" name="expense_date[]" class="form-control" value="{{$item->date}}" required>
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <div class="form-group">
                                    <label for="">Service charge</label>
                                    <input type="text" value="{{abs($item->amount)}}" name="expense_charge[]" class="form-control compute-input charge-input" onkeyup="addThousandSeparator(this)" required>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="expense_account" name="expense_account[]" required>
                                            ...
                                            @foreach ($expenses as $expense)
                                                <option value="{{ $expense->id }}" @if($item->chartOfAccount->id == $expense->id) selected @endif>{{ $expense->account_name }} ({{ $expense->account_code }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        @if ($item->type=='Interest')
                            <div class="row">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                    <label for="">Date</label>
                                    <input type="date" name="income_date[]" class="form-control" value="{{$item->date}}" required>
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <div class="form-group">
                                    <label for="">Interest Earned</label>          
                                    <input type="text" value="{{$item->amount}}" name="income_earned[]" class="form-control compute-input interest-input" onkeyup="addThousandSeparator(this)" required>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" name="income_account[]" required>
                                            ...
                                            @foreach ($incomes as $income)
                                                <option value="{{ $income->id }}" @if($item->chartOfAccount->id == $income->id) selected @endif>{{ $income->account_name }} ({{ $income->account_code }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fa fa-solid fa-save"></i> Update Reconcile</button>
                </form>
...
<script>
        ...
        $('#addInterestBtn').click(function() {
            ...
            var newRow = `<div class="row" id="mainInterests">
                <div class="col-sm-4">
                            <div class="form-group">
                              <label for="">Date</label>
                              <input type="date" name="income_date[]" class="form-control" placeholder="" required>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
    ...
    function getDifference()
    {
        ...
        $('.charge-input').each(function() {
            ...
            totalExpense += parseFloat(value); // Add the value to the total
        });
        let totalIncome=0;
        $('.interest-input').each(function() {
            var value = $(this).val() || 0; // Get the value or default to 0
            value = value.replace(/,/g, '');
            totalIncome += parseFloat(value); // Add the value to the total
        });
