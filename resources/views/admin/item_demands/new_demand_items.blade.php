        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="box-header-flex">
                    ...
                    <a href="{{route('demands.item-demands.new')}}" class="btn btn-primary">{{'<< '}} Back</a>
                </div>
            </div>
            ...
            <div class="box-body">
                ...
                <table class="table table-bordered" id="create_datatable_25">
                    ...
                    <tbody>
                    @foreach($demandItems as $index => $demand)
                        <tr>
                            <th style="width: 3%;" scope="row"> {{ $index + 1 }}</th>
                            <td> {{ $demand->created_at->format('Y-m-d') }} </td>
                            <td>{{ $demand->getInventoryItemDetail?->stock_id_code ?? '-' }}</td>
                            
                            <td>{{ $demand->getInventoryItemDetail?->title ?? '-' }}</td>
