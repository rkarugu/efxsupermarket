...
<th width="5%">#</th>	
<th >Item No.</th>
<th style="text-align: center  !important;">QoH</th>
<th style="text-align: right  !important;">Current Cost</th>
<th style="text-align: right  !important;">New Cost</th>
...
<td style="width:45% !important; ">{!! $delta->getInventoryItemDetail?->title !!}</td>
<td style="text-align: center;">{!! $delta->demand_quantity !!}</td>
<td style="text-align: right;" >{!! manageAmountFormat($delta->current_cost)!!}</td>
<td style="text-align: right;" >{!! manageAmountFormat($delta->new_cost) !!}</td>
<td style="text-align: right;" >{!! manageAmountFormat($delta->amount) !!}</td>
	</tr>
	<?php $total_amount[] = $delta->amount ?>
	<?php $qty[] = $delta->demand_quantity; ?>
	@endforeach
	<tr style="border-bottom: 1px solid black;border-top:1px solid black">
		<td colspan="5" style="text-align:center;font-weight:bolder;font-size:13px">Grand Total</td>
		<td style="text-align:right;font-weight:bolder;font-size:13px">{!! manageAmountFormat(array_sum($total_amount)) !!}</td>
	</tr>
@if ($demand?->edited_demand_amount > 0 && $demand?->edited_demand_amount != $total_amount)
<tr style="border-bottom: 1px solid black;border-top:1px solid black">
<td colspan="5" style="text-align:center;font-weight:bolder;font-size:13px">Approved Total</td>
<td style="text-align:right;font-weight:bolder;font-size:13px">{!! manageAmountFormat($demand->edited_demand_amount) !!}</td>
</tr>
@endif
</table>
...
<td colspan="4">Approved Total</td>
<td style="border-bottom: 1px solid; text-align:right;">{!! manageAmountFormat($demand->edited_demand_amount) !!}</td>
	</tr>
