                ...
                <form action="" method="get" id="filter-form">
                    <div class="row">
                        <div class="form-group col-md-3">
                            ...
                            <input type="date" name="startDate" id="startDate" class="form-control" value="{{ request()->startDate ?? \Carbon\Carbon::now()->toDateString() }}">
                        </div>
                        <div class="form-group col-md-3">
                            <label for="date"> End date </label>
                            <input type="date" name="endDate" id="endDate" class="form-control" value="{{ request()->endDate ?? \Carbon\Carbon::now()->toDateString() }}">
                        </div>

                        <div class="form-group col-md-3">
                            <label for="branch"> Filter by branch </label>
                            <select name="branch" id="branch" class="form-control">
                                <option value="" @if(!request()->branch) selected @endif disabled>Select branch</option>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select name="route" id="route" class="form-control">
                                <option value=""  @if(!request()->route) selected @endif disabled>Select route</option>
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <a href="{{ route('route-reports.weekly-sales') }}" class="btn btn-primary" id="reset-btn">  Reset </a>
                        </div>
                    </div>
                </form>
...
@section('uniquepagescript')
    ...
    <script>
        ...
        filterButton.addEventListener('click', function() {
            form.action = "{{ route('route-reports.weekly-sales') }}";
        });

        downloadButton.addEventListener('click', function() {
            form.action = "{{ route('route-reports.weekly-sales.download') }}";
        });
    });
