...
<section class="content">
    ...
    <div class="box box-primary">
        <div class="box-header with-border no-padding-h-b">
            <div  style="height: 150px ! important;"> 
                ...
                {!! Form::open(['route' => 'inventory-reports.supplier-product-reports2','method'=>'get']) !!}
                <div>
                    <div class="col-md-12 no-padding-h">
                        <div class="col-sm-3">
                            <div class="form-group">
                                <label for="start-date">Start Date:</label>
                                <input type="text" id="start-date" class="datepicker" name="start-date" autocomplete="off" value="{{@request()->get('start-date')}}">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <input type="text" id="end-date" class="datepicker" name="end-date" autocomplete="off" value="{{@request()->get('end-date')}}">
                            </div>
                        </div>

                        <div class="col-sm-3">
                            <div class="form-group">
                                {!!Form::select('supplier_id', getSuppliers(),NULL, ['class' => 'form-control mlselec6t','placeholder' => 'Select Supplier'])!!} 
                            </div>
                        </div>
        ...
        <tr style="text-align: left;">
            <th  colspan="8">Suppliers Product Movement</th>
        </tr>
    ...
    <!-- Dynamic code start -->
                ...
                @foreach($lists as $arr)
                    ...
                    <tr style="text-align: left;">
                        <td colspan="{{isset($selectedMonthArr['m']) && !is_null($selectedMonthArr['m'])?count($selectedMonthArr['m'])+2:''}}"><b>{{ $arr['name']}}</b></td>
                    </tr>
                    ...
                    @foreach($arr['products'] as $item)
                        ...
                        @php
                            ...
                            $itemstock=0;
                            $itemstockQry="";
                            if(isset($item->getstockmoves2) && !empty($item->getstockmoves2)){
                                
                                $itemstockQry=$item->getstockmoves2->where('wa_inventory_item_id',$item->id);
                         ...
                         @if(isset($selectedMonthArr['m']) && !is_null($selectedMonthArr['m']))
                            @foreach($selectedMonthArr['m'] as $key => $month)
                                ...
                                $year=$selectedMonthArr['y'][$key]; 
                                    ...
                                    $monthlyStock=0;
                                    if($itemstockQry!=""){
                                       $monthlyStock=$item->getstockmoves2->where('wa_inventory_item_id',$item->id)->whereRaw(' ( document_no LIKE "%INV%" OR document_no LIKE "%CS%" OR document_no LIKE "%RTN%" ) && DATE(created_at) >= "'.$created_from.'" && DATE(created_at) <= "'.$created_to.'"')->sum('qauntity'); 
                                    }
                ...
                <tr style="text-align: right;">
                    <td colspan="{{isset($selectedMonthArr['m']) && !is_null($selectedMonthArr['m'])?count($selectedMonthArr['m'])+2:''}}" style="text-align: left;">

                        <b>Total: {{ abs(array_sum($new_final_arr)) }} </b></td>
                </tr>
            </table>
