...
<section class="content">    
    <div class="box box-primary">
        {{-- <div class="box-header with-border"><h3 class="box-title"> Receive {!! $title !!} </h3></div> --}}
        <div class="box-header with-border">
          <div class="box-header-flex">
              <h3 class="box-title"> Receive Transfers </h3>
              <a href="{{route('n-transfers.indexReceive')}}" class="btn btn-primary">{{'<< '}} Back</a>
          </div>
      </div>
                           ...
                           <h3 class="box-title"> Items </h3>
                                ...
                                <table class="table table-bordered table-hover" id="mainItemTable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>                                      
                                            ...
                                            <td>
                                                 @if (isset($binLocations[$item->wa_inventory_item_id]))
                                                    {{ $binLocations[$item->wa_inventory_item_id] }}
                                                    <input type="hidden" class="bin-location" value="{{ $binLocations[$item->wa_inventory_item_id] }}">
                                                @else
                                                    <form action="{{ route('n-transfers.updateUnitOfMeasure', $item->id) }}" method="POST" class="      bin-form">
                                                @csrf
                                                    <select required name="wa_unit_of_measure_id" class="form-control mlselec6t">
                                                <option value="" disabled selected>Select Bin</option>
                                                @foreach($unitmeasures as $unitmeasure)
                                                    <option value="{{ $unitmeasure->id }}">{{ $unitmeasure->title }}</option>
                                                @endforeach
                                                  </select>
                                                    <input type="hidden" name="store" value="{{ $interBranchTransfer->to_store_location_id }}">
                                                     </form>
                                                 @endif
                                            </td>
                                            <td>{{ $item->quantity }}</td>
                              ...
                              <div class="col-md-6"><span>
                                @if ($interBranchTransfer->status == "PENDING")
                                     <a id="processReceiveButton" href="{{ route('n-transfers.processReceiveInterBranchTransfer', $interBranchTransfer->transfer_no) }}" class="btn btn-primary btn-sm">Process Receive</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const processReceiveButton = document.getElementById('processReceiveButton');
        const binForms = document.querySelectorAll('.bin-form');
        const binSelects = document.querySelectorAll('.bin-form select');
        const flashMessage = document.getElementById('flash-message');

        // Function to check if all bin locations are set
        function checkBinLocations() {
            let allBinsSet = true;
            ...
            processReceiveButton.disabled = !allBinsSet;
        }
        ...
        // Handle form submissions and button click
        processReceiveButton.addEventListener('click', function (event) {
            event.preventDefault(); 

            let allValid = true;

            binForms.forEach(form => {
                const select = form.querySelector('select[name="wa_unit_of_measure_id"]');
            ...
            if (allValid) {                
                window.location.href = processReceiveButton.getAttribute('href');
            } else {
                flashMessage.classList.add('alert', 'alert-danger');
                flashMessage.textContent = 'Please ensure all items have a bin location set before processing.';
            }
        });
    });
</script>
