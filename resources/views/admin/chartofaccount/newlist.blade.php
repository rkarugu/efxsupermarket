
@extends('layouts.admin.admin')

@section('content')
<style>
    .bg-grey th,.bg-grey td,.bg-grey{
        background-color: #eee;
    }
</style>

    <!-- Main content -->
    <section class="content">
        <!-- Small boxes (Stat box) -->
                    <div class="box box-primary">
                        <div class="box-header with-border no-padding-h-b">
                             @if(isset($permission[$pmodule.'___add']) || $permission == 'superadmin')
                            <div align = "right"> 
                                <a href = "{!! route($model.'.create')!!}" class = "btn btn-success">Add {!! $title !!}</a>
                                <a href = "{!! route('chart-of-accounts.downloadCoaitems')!!}" class = "btn btn-danger">Excel</a>
                            </div>
                             @endif
                            <br>
                            @include('message')
                            <div class="col-md-12 no-padding-h">
                                <table class="table table-bordered table-hover" id="create_datatable">
                                    <thead>
                                    <tr>
                                        <th width="10%">S.No.</th>
                                        <th width="15%"  >Account Group</th>
                                        <th width="15%"  >Account Section</th>
                                        <th width="15%"  >Account Sub Section</th>
                                        <th width="15%"  >Account Code</th>
                                        <th width="15%"  >Account Name</th>
                                        <th  width="15%" class="noneedtoshort" >Action</th>                                                                          
                                    </tr>
                                    </thead>

                                    <tbody>
                                    @if(isset($lists) && !empty($lists))
                                        <?php $b = 1;?>
                                        @foreach($lists as $list)
                                        <tr class="{{($b%2!=0) ? 'bg-grey' : NULL}}">
                                            <td>{!! $b !!}</td>     
                                            <td>{!!  
                                                $list->getRelatedGroup 
                                                ?
                                                ($list->getRelatedGroup->getAccountSection 
                                                ? $list->getRelatedGroup->getAccountSection?->section_name
                                                : NULL)
                                                : NULL
                                                !!}
                                            </td>
                                            <td>{!! $list->getRelatedGroup ? $list->getRelatedGroup->group_name : '-' !!}</td>     
                                            <td>{!! $list->getSubAccountSection?->section_name !!}</td>
                                            <td>{!! $list->account_code !!}</td>
                                             <td>{!! $list->account_name !!}</td>
                                            <td class = "action_crud">
                                            @if($list->slug != 'mpesa')
                                             @if(isset($permission[$pmodule.'___edit']) || $permission == 'superadmin')
                                                <span>
                                                <a title="Edit" href="{{ route($model.'.edit', $list->slug) }}" ><img src="{!! asset('assets/admin/images/edit.png') !!}" alt="">
                                                </a>
                                                </span>
                                                <span>
                                                <a title="Edit" href="{{ route($model.'.gl-trans', $list->account_code) }}?to={{date('Y-m-d')}}&from={{date('Y-m-d')}}" >
                                                    <i class="fa fa-list"></i>
                                                </a>
                                                </span>

                                                @endif

                                                 @if(canDeleteParentData() == true)

                                               @if(isset($permission[$pmodule.'___delete']) || $permission == 'superadmin')

                                                <span>
                                                <form title="Trash" action="{{ URL::route($model.'.destroy', $list->slug) }}" method="POST">
                                                <input type="hidden" name="_method" value="DELETE">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                <button  style="float:left"><i class="fa fa-trash" aria-hidden="true"></i>
                                                </button>
                                                </form>
                                                </span>
                                                 @endif
                                                  @endif


                                                 @endif





                                            </td>
                                            
                                        
                                        </tr>


                                           <?php $b++; ?>
                                        @endforeach
                                    @endif


                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>


    </section>
   
@endsection
