    ...
    <section class="content">
        ...
        <div class="box box-primary">
            <div class="box-header with-border no-padding-h-b">
                <div class="row">
                    <div class="col-md-9">
                        <h3>{{ $inventoryItem->title }} | Discounts</h3>
                    ...
                    <div class="col-sm-3">
                        @if(isset($permission[$pmodule.'___manage-discount']) || $permission == 'superadmin')
                            <div align="right"><a href="{!! route('discount-bands.create' , $inventoryItem->id)!!}" class="btn btn-success">Add Discount Band</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-md-12 no-padding-h">
                    <table class="table table-bordered table-hover" id="create_datatable_10">
                        ...
                        <tbody>
                            ...
                            {{-- <td>{{getUserData($discountBand->approved_by)->name ?? '-'}}</td> --}}
                            ...
                            <td>
                                ...
                                <a href="{{ route('discount-bands.edit', $discountBand->id)}}"><i class="fas fa-pen" title="edit"></i></a>
                                @endif

                                    
                                @if (isset($permission[$pmodule . '___approve-discount']) || $permission == 'superadmin')
                                @if ($discountBand->status != "APPROVED" )
                                <button type="button" class="text-primary mr-2 btn-decline transparent-btn" data-toggle="modal" title="Approve" data-target="#confirmationModal" data-discount-band-id="{{ $discountBand->id }}">
                                    <i class="fas fa-check"></i>
                                </button>
                                    
                                @endif
                                <button type="button"  class="text-primary mr-2 btn-decline2 transparent-btn" data-toggle="modal" title="Delete" data-target="#confirmationModal2" data-discount-band-id="{{ $discountBand->id }}">
    ...
    <div class="modal-content">
        ...
        <form method="POST" id="confirmationForm" action="">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary btn-submit-updated-center">Yes, Approve Discount</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="staticBackdropLabel">Are you sure you want to Delete this discount band?</h4>
        ...
        <form method="POST" id="confirmationForm2" action="">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                <button type="submit" class="btn btn-primary btn-submit-updated-center2">Yes, Delete Discount</button>
            </div>
        </form>
    </div>
</div>
