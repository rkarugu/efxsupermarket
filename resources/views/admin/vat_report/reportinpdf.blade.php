...
<table style = "border:1px solid gray" class="data_detail">
    ...
    <tr>
      ...
      <td style="text-align:right;">{{ (manageAmountFormat(abs($itemData['periodDebit']))=="0.00") ? ' - ' : manageAmountFormat(abs($itemData['periodDebit'])) }}</td>
      <td style="text-align:right;">{{ (manageAmountFormat(abs($itemData['periodCredit']))=="0.00") ? ' - ' : manageAmountFormat(abs($itemData['periodCredit'])) }}</td>
    </tr>
		<?php 
...
<tr style ="font-weight: bold;">
    <td></td>
    <td>Total</td>
    <td style="text-align:right;">{{ manageAmountFormat(abs(array_sum($periodDebit)))}}</td>
    <td style="text-align:right;">{{ manageAmountFormat(abs(array_sum($periodCredit)))}}</td>
</tr>
