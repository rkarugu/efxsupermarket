...
<section class="content">    
    <div class="box box-primary">
        <div class="box-header with-border">
         ...
         @include('message')
        <form class="validate form-horizontal"  role="form" method="POST" action="{{route('admin.stock-counts-users-assingment.update', $assignment->id)}}" enctype = "multipart/form-data">
            {{ csrf_field() }}
            <div class="box-body">
                <div class="form-group">
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            {!! Form::date('stock_take_date', $assignment->stock_take_date, ['maxlength'=>'255','placeholder' => '0', 'required'=>true, 'class'=>'form-control']) !!}  
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            @php
                            $assignee=[];
                                foreach ($assignment->assistant as $value) {
                            ...
                            <select name="user[]" id="user[]" class="mlselect form-control" multiple required>
                                ...
                                @foreach ($users as $user )
                                    <option value="{{$user->id}}" {{ in_array($user->id, $assignee) ? 'selected' : '' }}> {{ $user->name}} </option>                                
                                @endforeach
                            </select> 
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <select name="category[]" id="category[]" class="mlselect form-control" multiple required>
                                ...
                                <option value="{{ $category->id }}" {{ in_array($category->id, explode(',', $assignment->category_ids)) ? 'selected' : '' }}>
