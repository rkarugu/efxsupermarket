            ...
            <div class="box-body">
                ...
                <div class="col-md-12">
                    <table class="table table-bordered table-hover" id="create_datatable">
                        ...
                        <tbody>
                        @foreach($promotionGroups as $promotion_group)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $promotion_group->name }}</td>
                                <td>{{ $promotion_group->active?'Yes':'No' }}</td>
                                <td>{{ $promotion_group->start_time }}</td>
                                <td>{{ $promotion_group->end_time }}</td>
                                <td>
                                    <button class="btn btn-sm btn-info edit-btn" data-id="{{ $promotion_group->id }}" data-all="{{ $promotion_group }}" data-toggle="modal" data-target="#editModal">
                                        <i class="glyphicon glyphicon-pencil"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger delete-btn" data-id="{{ $promotion_group->id }}" data-name="{{ $promotion_group->name }}" data-toggle="modal" data-target="#deleteModal">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#createForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('promotion-group.store') }}",
                    method: "POST",
                    data: formData,
