            ...
            {{method_field('PATCH')}}
                     ...
                     <div class = "row">
                      <div class="box-body">
                          <div class="form-group">
                              ...
                              <div class="col-sm-7">
                                  {!! Form::text('manual_doc_no', $row->manual_doc_no, ['maxlength'=>'255','placeholder' => '', 'required'=>true, 'class'=>'form-control']) !!}  
                              </div>
                          </div>
                      </div>
                    </div>
             ...
             <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                          {!!Form::select('to_store_id',getStoreLocationDropdownByBranch($row->restaurant_id), $default_to_store_id, ['class' => 'form-control ','id'=>'to_store_id'  ])!!} 
                    </div>
                </div>
            </div>
                     </div>
                                         ...
                                         @foreach ($row->getRelatedItem as $item)
                                          ...
                                          <tr>                                      
                                             <td>
                                                 ...
                                                 <input style="padding: 3px 3px;" type="text" class="testIn form-control" value="{{@$item->getInventoryItemDetail->stock_id_code}}">
                                                 <div class="textData" style="width: 100%;position: relative;z-index: 99;"></div>
                                             </td>
                                             <td><input style="padding: 3px 3px;" readonly="" type="text" name="item_description[{{@$item->wa_inventory_item_id}}]" data-id="{{@$item->wa_inventory_item_id}}" class="form-control" value="{{@$item->getInventoryItemDetail->title}}"></td>
                                             ...
                                             <td><input style="padding: 3px 3px;" readonly="" type="text" name="item_unit[{{@$item->wa_inventory_item_id}}]" data-id="{{@$item->wa_inventory_item_id}}" class="form-control" value="{{@$item->getInventoryItemDetail->pack_size->title}}"></td>
                                             <td><input style="padding: 3px 3px;" onkeyup="getTotal(this)" onchange="getTotal(this)" type="number" name="item_quantity[{{@$item->wa_inventory_item_id}}]" data-id="{{@$item->wa_inventory_item_id}}" class="quantity form-control" value="{{$item->quantity}}"></td>
...
<script src="{{asset('assets/admin/dist/bootstrap-datepicker.js')}}"></script>
  ...
  $(document).on('keyup','.testIn',function(e){ var vale = $(this).val();
      ...
      $.ajax({
        type: "GET",
        url: "{{route('supreme-store-requisitions.inventoryItems')}}",
        data: {
          'search':vale
        },
  ...
  $(document).click(function(e){
    ...
    function fetchInventoryDetails(varia){
      ...
      if(furtherCall == true){
        $.ajax({
          type: "GET",
          url: "{{route('supreme-store-requisitions.getInventryItemDetails')}}",
          data: {
            'id':$this.data('id')
          },
          success: function (response) {
