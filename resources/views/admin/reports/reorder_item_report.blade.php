...
@section('content')
    <div class="content">
        <div class="box box-primary">
            ...
            <div class="box-header with-border">
                <div class="d-flex justify-content-between">
                    <h3 class="box-title">Reorder Items (Stock at Hand <= Reorder Level) Report</h3>
                </div>
            </div>

            <div class="box-body">
                <form action="{{ route('inventory-reports.reorder-items-report.index') }}" method="get">
                            ...
                            <select id="location" name="location" class="form-control">
                                @foreach ($locations as $location)
                                    <option value="{{ $location->id }}"
                                        @selected($location->id == request()->location ?? auth()->user()->wa_location_and_store_id)>
                            ...
                            <div class="col-sm-2">
                                <select id="user" name="user" class="form-control">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->user_id }}" @if (request()->user == $user->user_id)
                                            selected
                                        @endif>
        ...
        <div class="box">
            <div class="box-body">
                <table class="table table-bordered" id="reorderItemsDataTable">
                    <thead>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#reorderItemsDataTable").DataTable({
                processing: true,
                serverSide: true,
                scrollX: true,
                scrollY: "600px",
                ...
                ajax: {
                    url: '{!! route('inventory-reports.reorder-items-report.index') !!}',
                    data: function(data) {
        ...
        function refreshTable() {
            $("#reorderItemsDataTable").DataTable().ajax.reload();
        }
    </script>
@endpush
