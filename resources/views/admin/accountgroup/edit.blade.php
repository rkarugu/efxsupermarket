         ...
         {!! Form::model($row, ['method' => 'PATCH','route' => [$model.'.update', $row->slug],'class'=>'validate','enctype'=>'multipart/form-data']) !!}
            {{ csrf_field() }}
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                         {!!Form::select('parent_id', getParentAccountGroupsDropdown($row->id), null, ['class' => 'form-control','required'=>false,'placeholder' => 'Please select group' ,'id'=>'selector_selects2' ])!!} 
                    </div>
                </div>
            </div>
