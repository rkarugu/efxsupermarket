            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table class="table table-bordered table-striped" id="selectedInvoicesTable">
                        ...
                        <tbody>
                            <tr>
                                <td colspan="11">No invoices selected</td>
                            </tr>
                        </tbody>
                    </table>
                                    ...
                                    @forelse($paymentVouchers as $paymentVoucher)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('payment-vouchers.print_pdf', $paymentVoucher->id) }}" target="_blank">
...
@push('scripts')
    ...
    <script>
        ...
        function renderPendingInvoices(items) {
            if (items.length == 0) {
                return `<tr><td colspan="11">No invoices to be selected</td></tr>`;
            }
        ...
        function renderSelectedInvoices(items) {
            ...
            items.forEach(item => {
                records += `<tr id="invoce_` + item.id + `">
                                ...
                                <td class="text-right">` + Number(item.total_amount_inc_vat - item.withholding_tax).formatMoney() + `</td> 
                                <td class="text-center">
                    ...
                    if(item.notes) {
                        ...
                        notes.forEach(note => {
                            records += `<tr id="invoce_` + note.id + `">
                                ...
                                <td class="text-right">`+ (note.type == 'CREDIT' ? `-`:``) + Number(note.amount).formatMoney() + `</td> 
                                <td class="text-right">`+ (note.type == 'CREDIT' ? `-`:``) + Number(note.tax_amount).formatMoney() + `</td> 
                                <td class="text-right">`+ (note.type == 'CREDIT' ? `-`:``) + Number(note.withholding_amount).formatMoney() + `</td> 
                                <td class="text-right">`+ (note.type == 'CREDIT' ? `-`:``) + Number(note.amount - note.withholding_amount).formatMoney() +`</td> 
                                <td class="text-center">
                                </td>
                            </tr>`;
                        });                        
        ...
        function calculateToAllocate() {
            ...
            window.selectedInvoices.forEach(function(invoice) {
                ...
                if(invoice.notes.length > 0){
                    ...
                    balance = Number(invoice.total_amount_inc_vat) - Number(invoice.withholding_tax) + noteAmount;                  
                } else {
                    balance = Number(invoice.total_amount_inc_vat) - Number(invoice.withholding_tax);
                }
