        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="d-flex justify-content-between align-items-center">
                    <h3 class="box-title">    {{ $shift->shift_id }} Shift Reported Issues </h3>

                    <a class="btn btn-primary"   href="{{ route("salesman-shift.index") }}"
                           > Return Back
                    </a>
                </div>
            </div>
                    ...
                    <div class="col-md-12 no-padding-h">
                        <table class="table table-bordered table-hover" id="create_datatable">
                            ...
                            @foreach ($issues as $issue)
                                <tr>
                                    <td>{{ $issue->responseCustomer->contact_person }}</td>
                                    <td>{{ $issue->responseCustomer->phone }}</td>
                                    <td>{{ $issue->responseCustomer->bussiness_name }}</td>
                                    <td>{{ $issue->response_options->count() }}</td>
                                    <td>{{ $issue->response_options->count() }}</td>
                                    <td>{{ $issue->response_options->count() }}</td>
                                    <td>{{ $issue->created_at->format('M. d Y, h:i A') }}</td>
                                    <td>
                                        <div class="action-button-div">
                                            <a href="{{ route('salesman-shift.reported-issue-details', $issue->id) }}"
                                               class="text-primary" title="View Reported Issues">
                                                <i  class='fa fa-list-alt text-danger fa-lg'></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
