    ...
    <section class="content">
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <form action="{{ route('transfers.returns.process_return_pending_late_return') }}" method="post" id='return-form'>
                    {{ csrf_field() }}
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                ...
                                @if ($returnItem->current_price < $returnItem->sp)
                                    <td style="text-align: right; background-color:red; color:white; font-weight:bold;">{{  number_format($returnItem->current_price, 2) }}</td>
                                    <td style="text-align: right; background-color:red; color:white; font-weight:bold;">{{ number_format($returnItem->sp, 2) }}</td>
                                @elseif ($returnItem->current_price > $returnItem->sp)
                                    <td style="text-align: right; background-color:#31a5ec; color:white; font-weight:bold;">{{  number_format($returnItem->current_price, 2) }}</td>
                                    <td style="text-align: right; background-color:#31a5ec; color:white; font-weight:bold;">{{ number_format($returnItem->sp, 2) }}</td>
                                @else
                                    <td style="text-align: right; ">{{  number_format($returnItem->current_price, 2) }}</td>
                                    <td style="text-align: right; ">{{ number_format($returnItem->sp, 2) }}</td>
                                @endif

                                <td style="text-align: right;" >{{ number_format(($returnItem->return_quantity * $returnItem->sp),2) }}</td>
                                <td>
        ...
        $.ajax({
            ...
            $('#loader-on').hide();
             if(result.status === 1) {
                    ...
                    window.location.href = "{{route('transfers.return_list_groups_late_returns')}}";
                }
             if(result.status === 0) {
