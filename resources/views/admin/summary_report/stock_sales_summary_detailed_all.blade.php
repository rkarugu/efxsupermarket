        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="box-header-flex">
                    ...
                    <div>
                        <a href="{{route('detailed-sales-summary-report.stock-sales-excel-download', ['date'=>$date])}}" class="btn btn-primary btn-sm" ><i class="fas fa-file-excel" ></i>  Export</a>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($salesData as $sale)
                                <tr>
                                    ...
                                    <td style="text-align:right;">{{manageAmountFormat($sale->taxable_sales - $sale->total_tax)}}</td>
                                ...
                                @php
                                    $grand_total_vatable += ($sale->taxable_sales - $sale->total_tax);
                        ...
                        <tbody>
                            ...
                            @foreach ($returnsData as $sale)
                                <tr>
                                    <th>{{$loop->index+1}}</th>
                                    <td>{{$sale->document_no}}</td>
                                    <td style="text-align:right;">{{manageAmountFormat($sale->taxable_returns - $sale->total_tax)}}</td>
                                    <td style="text-align:right;">{{manageAmountFormat($sale->total_tax)}}</td>
                                    <td style="text-align:right;">{{manageAmountFormat($sale->total_sales)}}</td>
                                </tr>
                                @php
                                    $grand_total_vatable += ($sale->taxable_returns - $sale->total_tax);
                                    $grand_total_vat += $sale->total_tax;
                                    $grand_total_sales += $sale->total_sales;
                                @endphp
                            @endforeach
