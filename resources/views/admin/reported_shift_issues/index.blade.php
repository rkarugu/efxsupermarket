...
@section('content')
    <section class="content">
                    ...
                    <div class="box-body">
                        ...
                        <div class="row w-100" style="margin-top: 20px">
                            <div class="col-md-6">
                                ...
                                <img id="onboarded-image" src="" alt="Onboarded Image" width="100%;">
                            </div>
                            <div class="col-md-6">
            ...
            <div class="modal-dialog" role="document">
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <div id="show-price-conflict-data">
                            <div class="row">
                                <div class="col-md-6">
                                    <p style="font-weight:bolder">Taken Photo</p>
                                    <img id="reported-price-issue-image" src="" alt="Issue Image"
                                        width="200px;height:200px;object-fit:cover">
                                </div>
                            </div>
                            <hr>
                        ...
                        @if ($logged_user_info->role_id == 1 || isset($my_permissions['reported-shift-issues___hq-comment']))
                        <div class="row" style="margin-top: 10px">
                            <div class="col-md-12">
                                <label id="hq-reported-resolved-description-title" style="width: 100%">HQ Comment</label>
                                <textarea name="hq-reported-resolved-description" id="hq-reported-resolved-description" style="width: 100%"
                                    rows="5"></textarea>
                            </div>
                        </div>
                        @endif
                    ...
                    <div class="box-footer">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <button
                                    type="button"
                                    class="btn btn-primary submit-resolution"
                                    data-type="with-message"
                                >
                                    Submit With Message
                                </button>
                                <button
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title"> Salesman Shift Reported Issues </h3>
            </div>

            <div class="box-body">

                <form action="{{ route('reported-shift-issues.index') }}" method="get">
...
@section('uniquepagestyle')
    ...
    <style>
        ...
        .hq-reported-resolved-description {
            vertical-align: top;
            padding: 5px;
            box-sizing: border-box;
            overflow: auto;
        ...
        $('#view-issue-modal').on('show.bs.modal', function(event) {
            ...
            if (dataValue.display_scenario == 'No Order') {
                ...
                if (dataValue.image == null) {
                    ...
                    $('#issue-image').attr('src', '{{ asset('uploads/shift_issues/') }}/' + dataValue.image)
                        .show();
                    $('#onboarded-image').attr('src', '{{ asset('uploads/shops/') }}/' +
                            dataValue
                            .original_image)
                        .show();
                }
        ...
        $(document).ready(function() {
            ...
            $('#resolve-issue-modal').on('show.bs.modal', function(event) {
                ...
                if (dataValue.display_scenario == 'No Order') {
                    ...
                    $('#hq-reported-resolved-description').text(dataValue.hqResolvedDescription);
                    $('#reported-issue-scenario, #reported-issue-route, #reported-issue-salesman, #reported-issue-customer, #reported-resolved-description, #reported-resolved-description-title, #hq-reported-resolved-description, #hq-reported-resolved-description-title, #reported-issue-scenario-title, #reported-issue-route-title, #reported-issue-salesman-title, #reported-issue-customer-title')
                        .show();
                    $('#show-shop-closed-data, #show-price-conflict-data, #reported-product-name, #reported-issue-original-image, #reported-new-price, #reported-selling-price, #reported-issue-image, #reported-item-description, #reported-product-name-title, #reported-new-price-title, #reported-selling-price-title, #reported-issue-image-title, #reported-item-description-title')
                        .hide();
                } else if (dataValue.display_scenario == 'Price Conflict') {
                    $('#reported-issue-scenario').text(dataValue.display_scenario);
                    ...
                    $('#reported-resolved-description').text(dataValue.resolvedDescription);
                    $('#hq-reported-resolved-description').text(dataValue.hqResolvedDescription);
                    $('#reported-product-name, #show-shop-closed-data, #reported-issue-original-image, #reported-issue-route, #reported-issue-salesman, #reported-issue-customer, #reported-issue-route-title, #reported-issue-salesman-title, #reported-issue-customer-title, #reported-product-name-title')
                        .hide();
                    if (dataValue.image == null) {
                        $('#reported-price-issue-image').hide();
                    } else {
                        $('#reported-price-issue-image').attr('src',
                                '{{ asset('uploads/shift_issues/') }}/' +
                                dataValue
                                .image)
                            .show();
                    }
                    $('#reported-issue-scenario, #show-price-conflict-data, #reported-resolved-description, #reported-resolved-description-title, #hq-reported-resolved-description, #hq-reported-resolved-description-title, #reported-item-description, #reported-new-price, #reported-selling-price, #reported-issue-scenario-title, #reported-item-description-title, #reported-new-price-title, #reported-selling-price-title')
                        .show();
                } else if (dataValue.display_scenario == 'Shop Closed') {
                    $('#reported-issue-scenario').text(dataValue.display_scenario);
                    $('#reported-issue-route').text(dataValue.route);
                    ...
                    $('#reported-product-name, #show-price-conflict-data, #reported-new-price, #reported-selling-price, #reported-item-description, #reported-new-price-title, #reported-selling-price-title, #reported-item-description-title, #reported-product-name-title')
                        .hide();
                    $('#reported-issue-scenario, #show-shop-closed-data, #reported-resolved-description, #reported-resolved-description-title, #hq-reported-resolved-description, #hq-reported-resolved-description-title, #reported-issue-route, #reported-issue-salesman, #reported-issue-customer, #reported-issue-scenario-title, #reported-issue-route-title, #reported-issue-salesman-title, #reported-issue-customer-title')
                        .show();
                } else if (dataValue.display_scenario == 'New Product') {
                    $('#reported-issue-scenario').text(dataValue.display_scenario);
                    ...
                    $('#reported-resolved-description').text(dataValue.resolvedDescription);
                    $('#hq-reported-resolved-description').text(dataValue.hqResolvedDescription);
                    $('#show-shop-closed-data, #show-price-conflict-data, #reported-new-price, #reported-issue-original-image, #reported-selling-price, #reported-item-description, #reported-new-price-title, #reported-selling-price-title, #reported-item-description-title')
                        .hide();
                    if (dataValue.image == null) {
                    ...
                    $('#reported-product-name, #reported-resolved-description, #reported-resolved-description-title, #hq-reported-resolved-description, #hq-reported-resolved-description-title, #reported-issue-route, #reported-issue-salesman, #reported-issue-customer, #reported-product-name-title, #issue-route-title, #reported-issue-salesman-title, #reported-issue-customer-title')
                        .show();
                }
            ...
            $(document).on('click', '.submit-resolution', function(event) {
                ...
                let issueData = {
                    ...
                    hqResolvedDescription: $('#hq-reported-resolved-description').val(),
                    issueId: $('#reported-issue-id').val(),
                    productId: $('#product_id').val(),
                    submitType: event.target.dataset.type
                };
            ...
            $('#resolve-issue-modal').on('hidden.bs.modal', function() {
                ...
                $('#hq-reported-resolved-description').val('');
            });
        });
    </script>
@endsection
