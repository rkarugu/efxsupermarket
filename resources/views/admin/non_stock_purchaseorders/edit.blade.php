        ...
        <div class="box-header with-border"><h3 class="box-title"> {!! $title !!} </h3></div>
         ...
         {!! Form::model($row, ['method' => 'PATCH','route' => [$model.'.update', $row->slug],'class'=>'','enctype'=>'multipart/form-data']) !!}
           ...
           <div class="form-group">
               ...
               <div class="col-sm-7">
                   {!! Form::text('vehicle_reg_no', NULL, ['maxlength'=>'255','placeholder' => '','id'=>'vehicle_reg_no', 'class'=>'form-control','readonly'=>true]) !!}  
               </div>
           </div>
       </div>
       </div>
            ...
            <div class="col-sm-7">
             ...
             <div class="box-body">
                <div class="form-group">
                    ...
                    <div class="col-sm-6">
                    <select name="wa_department_id" id="wa_department_id" class="form-control departments" readonly>
                    @if($row->getDepartment)
                            <option value="{{$row->wa_department_id}}">{{@$row->getDepartment->department_name}}</option>
                          @endif
...
<!--                               <button type="button" class="btn btn-success" style="margin-bottom:5px" data-toggle="modal" data-target="#addRequisitionItemModel">Add Multiple Item To Purchase Order</button> -->
                              {{-- <button type="button" class="btn btn-success" data-toggle="modal" data-target="#addRequisitionItemModel">Add Item To Purchase Order</button> --}}

                              </span>
                              @if($row->status == 'UNAPPROVED' && $row->getRelatedItem && count($row->getRelatedItem)>0)
                                 <a href = "{{ route('purchase-orders.sendRequisitionRequest',$purchase_no)}}" style="margin-bottom:5px" class= "btn btn-success " >Send Request</a>
                                 @endif
