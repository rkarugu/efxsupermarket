...
@section('content')
    ...
    @if(request()->tyre_inventory_id)
        <section class="content">
            <!-- Small boxes (Stat box) -->
                <form method="post" action="{{route('tyre_retreading.store')}}" class="submitMe">
                        {{csrf_field()}}
                        {{method_field('POST')}}
                        <div class="box box-primary">
                            <div class="box-header with-border no-padding-h-b">
                                ...
                                <div class="col-md-12 no-padding-h table-responsive">
                                    <div class="col-md-3">
                                        <div class="form-group" >
                                           <label for="">Retread - Supplier</label>
                                           {!!Form::select('wa_supplier_id', getSupplierDropdown(),request()->wa_supplier_id, ['class' => 'form-control mlselec6t tyre_list','required'=>true,'placeholder' => 'Please select','id'=>'wa_supplier_id' ])!!} 
                                        </div>
                                    </div>
                                            ...
                                            @if($tyre_serials->count()>0)
                                                @foreach($tyre_serials as $key => $serial)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="text" class="form-control" placeholder="0.00" name="retread_cost[{{$serial->id}}]">
                                                        </td>
