...
@section('content')
 {!! Form::model($row, ['method' => 'PATCH','route' => ['stock-return.update', $row->slug],'class'=>'validate','enctype'=>'multipart/form-data']) !!}
            {{ csrf_field() }}
          <input type="hidden" name="grn" value="{{$row->grn_number}}">
                            ...
                            <span id = "requisitionitemtable">
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                      ...
                                      $total_with_vat_arr = [];
                                      ?>
                                        @foreach($grn as $items)
                                       
                                        <span id = "{{ @$items->id }}" class= "rendered_id"></span>

                                        <input type ="hidden" name = "purchase_order_ids[]" value = "{{ $items->wa_purchase_order_item_id }}" >
                                        <tr>
                                                                               <td style="width: 50px" >{{ $i }}</td>
                                                                               <td >{{ @$items->getRelatedInventoryItem->item_type }}</td>

                                                                       
                                          <td >{{ @$items->getRelatedInventoryItem->getInventoryItemDetail->getInventoryCategoryDetail->category_description  }}</td>
                                          <td >{{ @$items->getRelatedInventoryItem->getInventoryItemDetail->stock_id_code }}</td>
                                          <td >{{ @$items->getRelatedInventoryItem->getInventoryItemDetail->title }}</td>
                                      
                                            <td >{{ @$items->getRelatedInventoryItem->getSupplierUomDetail->title }}</td>
                                            <td class="align_float_right">{{ @$items->qty_invoiced }}</td>
                                          
                                                <td>
                                                  {{ @$items->qty_received}}
                                                </td>
                                                <td>
                                                  {!! Form::text('delivered_quantity_'.@$items->id,  0 , ['min'=>'0','max'=> @$items->qty_received,'class'=>'form-control delivered_quantity','id'=>'delivered_quantity_'.@$items->id,'data'=>@$items->id]) !!}  

                                                </td>
                                           


                                            <td >{{ @$items->getRelatedInventoryItem->getInventoryItemDetail->getUnitOfMeausureDetail->title }}</td>
                                          
                                            
                                          <td class="align_float_right">{{ @$items->getRelatedInventoryItem->unit_conversion }}</td>
                                        ...
                                        <td class="align_float_right">{{ @$items->getRelatedInventoryItem->quantity }}</td>
                                        <td class="">
                                        

                                        
                                                                                
                                          {!! Form::number('order_price_'.@$items->id,  @$items->getRelatedInventoryItem->order_price , ['min'=>'0','class'=>'form-control order_price','id'=>'order_price_'.@$items->id,'data'=>@$items->id,'required'=>true,'readonly'=>true]) !!} 
                                          ...
                                          <td>
                                           ...
                                           {!! Form::number('supplier_discount_'.@$items->id,  (@$items->getRelatedInventoryItem->stockMove ? @$items->getRelatedInventoryItem->stockMove->discount_percent : ''), ['max'=>100,'min'=>'0','class'=>'form-control supplier_discount','id'=>'supplier_discount_'.@$items->id,'data'=>@$items->id,'required'=>true,'readonly'=>true]) !!}  
                                        ...
                                        <td class="align_float_right" id="total_price_{{ @$items->id }}">
                                         
                                          {{ @$nett }}
                                         
                                        </td>
                                        <td class="align_float_right" id="vat_rate_{{ @$items->id }}"> {{ @$invoice_info->vat_rate }}
                                        </td>
                                        <td class="align_float_right" id="vat_amount_{{ @$items->id }}">
                                             
                                            {{ @$vat_amount }}
                                          
                                        </td>
                                        <td class="align_float_right" id="total_cost_with_vat_{{ @$items->id }}">
                                         
                                          {{ @$nett }}
                                            @php
                                                $total_with_vat_arr[] = @$nett;
                                            @endphp
                                        ...
                                        <td colspan="5">  
                                          ...
                                          <button type= "submit" class="btn btn-success">Process Goods Return</button></td>
                                          @endif
