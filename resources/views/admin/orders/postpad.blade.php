...
@section('content')
                                        ...
                                        @foreach($lists as $list)
                                            ...
                                            <tr>
                                                ...
                                                <td>{!! manageOrderidWithPad($list->id) !!}</td>
                                                 ...
                                                 $bill_id = getBillIdByOrderId($list->id);
            ...
            <td>{!! ucfirst($list->getAssociateRestro->name) !!}</td>
             ...
             <td>{!! str_replace('_',' ',$list->status) !!}</td>
                                                ...
                                                @if(isset($permission[$pmodule.'___edit']) || $permission == 'superadmin')
                                                    <span>
                                                    <a title="Edit Discount" href="{{ route('admin.edit.postpad.orders', $list->slug) }}" ><img src="{!! asset('assets/admin/images/edit.png') !!}" alt="">
                                                    </a>
                                                    </span>
                                                    ...
                                                    <span>
                                                    <a title="Void Order" href="javascript:void(0);" row_id="<?= $list->slug ?>" order_no="<?=  manageOrderidWithPad($list->id) ?>" onclick="setDeleteModal(this);">
                                                    ...
                                                    @if(isset($permission[$pmodule.'___cancle']) || $permission == 'superadmin')
                                                    <span>
                                                    <a title="Delete Order" href="{{ route('admin.cancle-order', $list->slug) }}"  onclick="return confirm('Do you really want to delete the order?')">
                                                    <i aria-hidden="true" class="fa fa-trash" style="font-size: 20px;"></i>
                                                    </a>
                                                    </span>
                                                     @endif
                                                     ...
                                                     */ ?>
                                                     ...
                                                     @if( $list->status == 'COMPLETED' && (isset($permission[$pmodule.'___transfer']) || $permission == 'superadmin'))
                                                    ...
                                                    <span>
                                                    <a title="Transfer Order" href="{{ route('admin.get.transfer-order', $list->slug) }}"  onclick="return confirm('Do you really want to transfer the order?')">
                                                  ...
                                                  <span>
                                                    <a title="Print Docket" href="javascript:void(0)" onclick="printBill({!! $list->id!!},{!! $logged_user_info->id !!},'D')"><i aria-hidden="true" class="fa fa-gitlab" style="font-size: 20px;"></i>
                                                     ...
                                                     <!--span>
                                                    <a style="font-size: 18px;" title="Show" href="{{ route($model.'.show', $list->slug) }}" ><i class="fa fa-eye" aria-hidden="true"></i>
                                    ...
                                    @endif
                                     ...
                                     <tfoot>
                                      ...
                                      <td style="font-weight: bold;">{{ manageAmountFormat($lists->sum('order_final_price')) }}</td>
                                      <td></td>
                                      <td></td>
    ...
    <div id="delete-order-with-reason-modal" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Void Order <span id="delete-popup-order-no"> </span></h4>
      </div>
      <div class="modal-body">

        <form class="validate"  role="form" method="POST" action="{{ route('admin.postpad.orders.delete_with_reason') }}" enctype = "multipart/form-data">
            {{ csrf_field() }}

            
                {!! Form::hidden('order_slug', '', ['id'=>'order_slug_delete']) !!}  
               <div class="box-body">
                <div class="form-group">
                    <label for="inputEmail3" class="col-sm-3 control-label">Reason</label>
                    <div class="col-sm-9">
                       {!! Form::textarea('order_cancle_reason', null, ['maxlength'=>'1000', 'class'=>'form-control', 'required'=>true]) !!}  
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
     /*  function printBill(order_id,user_id,print_type)
      ...
      function printBill(order_id,user_id,print_type)
       ...
       function setDeleteModal(row_obj){
            
            row_id  = $(row_obj).attr('row_id');
            order_no  = $(row_obj).attr('order_no');
            
            $('#order_slug_delete').val(row_id);
            $('#delete-popup-order-no').html('(Order No: '+order_no+ ')');
            
            $('#delete-order-with-reason-modal').modal('show');
       }
   </script>
@endsection
