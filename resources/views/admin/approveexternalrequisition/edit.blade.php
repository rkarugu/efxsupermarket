...
<div class="box-body">
  <div class="form-group">
      ...
      <div class="col-sm-6">
          <select name='wa_priority_level_id' class='form-control wa_priority_level_id' disabled="disabled">
            <option value="{{$row->wa_priority_level_id}}">{{@$row->priority_level->title}}</option>
          </select> 
      </div>
  </div>
</div>
...
<div class="box-body">
  <div class="form-group">
      ...
      <div class="col-sm-7">
          {!! Form::text('note', $row->note, ['maxlength'=>'255','placeholder' => '', 'class'=>'form-control','readonly'=>true]) !!}  
      </div>
  </div>
</div>
</div>
                ...
                <div class="form-group">
                   ...
                   <div class = "row">
                            <div class="box-body">
                                ...
                                <div class="col-sm-6">
                                {!!Form::select('store_location_id', getStoreLocationDropdownByBranch($default_branch_id),$default_wa_location_and_store_id, ['class' => 'form-control store_location_id','required'=>true,'placeholder' => 'Please select store','disabled'=>true ])!!} 
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class = "row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-6">
                                    {!!Form::select('wa_unit_of_measures_id', getUnitOfMeasureList(), $default_unit_of_measures_id, ['class' => 'form-control wa_unit_of_measures_id','required'=>true,'placeholder' => 'Please select bin location','disabled'=>true ])!!} 
                                </div>
                            </div>
                        </div>
                    </div>
                     ...
                     <div class = "row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-6">
                                    {!!Form::select('wa_supplier_id', getSuppliers(), $row->wa_supplier_id, ['class' => 'form-control wa_supplier_id mlselec6t','required'=>true,'placeholder' => 'Please select supplier','disabled'=>true ])!!} 
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    @if($row->getRelatedItem && count($row->getRelatedItem)>0)
                                      ...
                                      ?>
                                       ...
                                       {{--  <td >{{ @$getRelatedItem->getInventoryItemDetail->getInventoryCategoryDetail->category_description  }}</td>--}}
                                        ...
                                        ->where('wa_location_and_stores_id',$default_wa_location_and_store_id)
                                        ->where('wa_inventory_item_id',$getRelatedItem->getInventoryItemDetail->id)
                                        ->first();
                                        @endphp
                                        <td>{{@$getRelatedItem->getInventoryItemDetail->getAllFromStockMoves->where('wa_location_and_store_id',$default_wa_location_and_store_id)->sum('qauntity')}}</td>
                                      ...
                                      <td >{{@$stock_manage->max_stock}}</td>
                                        ...
                                        <td class = "action_crud">
                                                    ...
                                                    <span>
                                                      ...
                                                      <span>
                                                    <a title="Trash" href="{{ route('approve-external-requisitions.items.delete',[$row->purchase_no,$getRelatedItem->id])}}" ><i class="fa fa-trash" aria-hidden="true"></i>
                                                    </a>
                                                    </span>

                                                    <span>
                                                    <a class="left-padding-small" data-href="{!! route('approve-external-requisitions.editPurchaseItem',[ $purchase_no,$getRelatedItem->id]) !!}" onclick="editRequisitionItem('{!! route('approve-external-requisitions.editPurchaseItem',[ $purchase_no,$getRelatedItem->id]) !!}')"  data-toggle="modal" data-target="#edit-Requisition-Item-Model" data-dismiss="modal" ><i class="fa fa-edit" style="color: #444; cursor: pointer;" title="Edit"></i></a></span>
