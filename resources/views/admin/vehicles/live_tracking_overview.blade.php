...
<script>
    $(document).ready(function() {
        ...
        const notificationSoundPath = '{{ asset("assets/sounds/overspeedingNotificationSound.mp3") }}';
    ...
    <script type="text/javascript">
            ...
            class Popup extends google.maps.OverlayView {
                ...
                draw() {
                    ...
                    if (display === "block") {
                        ...
                        this.containerDiv.style.top = (divPosition.y - this.containerDiv.offsetHeight - 9) + "px"; // vetical distance btwn marker and popup 
                ...
                return $.ajax({
                    ...
                    success: function (response) {
                        ...
                        response.results.forEach(location => {
                                ...
                                // icon.innerHTML = `<img src="{{ asset('assets/admin/images/lorry.png') }}" height="30px" width="15px" />`;
                                icon.innerHTML = `<i class="fas fa-location-arrow truck-icon"></i>`;

                                let backgroundColor, status;
                                if(location.is_offline){
                                    icon.classList.add("grey-vehicle");
                                ...
                                const rotation = location.direction - 45; // Use this when an original icon is already rotated
                                // const rotation = location.direction  // use this for an up-right icon i.e  0 degrees rotation
                    ...
                    success: function (response) {
                        ...
                        if(canswitchIgnitionStatus){
                            if(response.switch_off_status === 'off') {
                                switchButtonHtml = `<a href="#" class="btn btn-sm switch-button" data-action="switchOn" data-url="${url}" data-device-name="${response.device_name}" style="margin-top:10px; color:white; background-color:green; border-color:blue;">Switch On</a>`;
                            } else {
            ...
            async function initMap() {
                ...
                setTimeout(() => {
                    echo.private('vehicle-location')
                    .listen('.VehicleLocationUpdated', (e) => {
                        const location = e;
                        
                        let existingMarker = markers.find(marker => marker.vehicle_name === location.device_number);
                        
                        if (existingMarker) {
                            const existingMarkerTimestamp = new Date(existingMarker.timestamp);
                            const locationTimestamp = new Date(location.timestamp);
                            if (existingMarkerTimestamp < locationTimestamp) {
                            const newPosition = new google.maps.LatLng(location.latitude, location.longitude);
                            existingMarker.position = newPosition;
                            const newRotation = location.direction - existingMarker.rotation - 45; 
                            existingMarker.content.style.transform = `rotate(${newRotation}deg)`;


                            let backgroundColor, status;
                            if(location.is_offline){
                                existingMarker.content.className = "grey-vehicle";
                                backgroundColor = "grey";
                                status = "offline";

                            }else if(location.movement && location.speed > 8 && location.speed <= 65){
                                existingMarker.content.className = "blue-vehicle";
                                backgroundColor = "blue";
                                status = "moving";

                            }else if(location.movement && location.speed > 65){
                                existingMarker.content.className = "red-vehicle";
                                backgroundColor = "red";
                                status = "overspeeding";    

                            }else if( location.speed <= 8 && location.ignition_status === 'ON' ){
                                existingMarker.content.className = "olive-vehicle";
                                backgroundColor = "olive";
                                status = "idling";

                            }else if (location.ignition_status === 'OFF'){
                                existingMarker.content.className = "green-vehicle";
                                backgroundColor = "green";
                                status = "stationery";
                            }
                            existingMarker.timestamp = location.timestamp;
                            existingMarker.rotation = newRotation;
                            existingMarker.popup.containerDiv.style.backgroundColor = backgroundColor;
                            ...
                            let vehicle_name = location.device_number;
                            let url = baseUrl.replace(':deviceName', vehicle_name);
                            const icon = document.createElement("div");
                            icon.className = "price-tag";
                            ...
                            const rotation = location.direction - 45;
                            // const rotation = location.direction
                            icon.style.transform = `rotate(${rotation}deg)`;

                            const content = document.createElement("div");
                            content.innerHTML = `<div class="device-name">${location.device_number}</div>`;
                            const popup = new Popup(
                                new google.maps.LatLng(location.latitude, location.longitude),
                                content,
                                backgroundColor
                            ...
                            const marker = new AdvancedMarkerElement({
                                ...
                                title: location.device_number,
                                content: icon,
                            });

                            marker.addListener("click", ()=>{
                                fetchVehicleInfoWindowDetails(location.device_number, marker);
                            });
                            ...
                            marker.vehicle_name = location.device_number;
                            marker.status = status; 
                            marker.popup = popup;
                            marker.timestamp = location.timestamp;
                            marker.rotation = rotation;
                    ...
                    function updateVehicleCounts() {
                        ...
                        $('#moving-vehicles-count').text(counts.moving);
                        $('#overspeeding-vehicles-count').text(counts.overspeeding);
                        $('#idling-vehicles-count').text(counts.idling);
                        $('#stationery-vehicles-count').text(counts.stationery);
                        $('#offline-vehicles-count').text(counts.offline);
                    }
